...
(function () {
    ...
    /* ── Loader helpers ── */
    /* ── Loader hide ── */
    var BZ_LOADER_MIN_MS = 2000;   // splash stays visible for at least this long after a page load/refresh
    var _bzLoaderHideTimer = null;
    function loaderHide() {
        // performance.now() counts from the start of navigation, so this holds the
        // splash until 2s have passed since the refresh began, then hides it.
        var wait = BZ_LOADER_MIN_MS - performance.now();
        if (wait > 0) {
            if (!_bzLoaderHideTimer) {
                _bzLoaderHideTimer = setTimeout(function () { _bzLoaderHideTimer = null; loaderHide(); }, wait);
            }
            return;
        }
    ...
    function startApp() {
        ...
        /* ═════════ RESUME GUARD v2 — shared helpers (used by the restore + play paths) ═════════ */
        window.BZ_RESUME_BUILD = 'resume-guard-v2';
        window._bzRDebug = (function () {
            try { return /[?&]bzdebug=1/.test(location.search) || localStorage.getItem('beatZen_debug') === '1'; } catch (_) { return false; }
        })();
        // Tiny on-device logger. Open the app with  ?bzdebug=1  to see a live green log box at the
        // bottom of the screen (works on phones, no DevTools needed). Silent otherwise.
        window._bzRLog = function (msg) {
            try {
                const buf = (window._bzRLogBuf = window._bzRLogBuf || []);
                buf.push(new Date().toTimeString().slice(0, 8) + ' ' + msg);
                if (buf.length > 60) buf.shift();
                if (!window._bzRDebug) return;
                console.log('[BZ-Resume]', msg);
                let box = document.getElementById('bz-resume-debug');
                if (!box) {
                    box = document.createElement('pre');
                    box.id = 'bz-resume-debug';
                    box.style.cssText = 'position:fixed;left:4px;right:4px;bottom:4px;max-height:38vh;overflow:hidden;margin:0;padding:6px 8px;background:rgba(0,0,0,.85);color:#7CFC98;font:10px/1.35 monospace;z-index:2147483647;pointer-events:none;white-space:pre-wrap;border-radius:8px;';
                    (document.body || document.documentElement).appendChild(box);
                }
                box.textContent = buf.slice(-16).join('\n');
            } catch (_) { /* logging must never break playback */ }
        };
        window._bzRLog('build ' + window.BZ_RESUME_BUILD + ' loaded');

        // The independent v2 record (beatZen_resume_v2) survives the many places that delete or
        // overwrite the legacy beatZen_lastPosition key. If the legacy key is missing / belongs to
        // another song / is <= 2s, rebuild it from v2 so the existing restore + UI paint just work.
        window._bzHealResumeStore = function (url, songId) {
            try {
                const v2 = JSON.parse(localStorage.getItem('beatZen_resume_v2') || 'null');
                if (!v2 || !url || v2.u !== url || !(Number(v2.t) > 2)) return;
                let legacyOk = false;
                try {
                    const p = JSON.parse(localStorage.getItem('beatZen_lastPosition') || 'null');
                    const lt = (p && typeof p === 'object') ? parseFloat(p.t) : parseFloat(p);
                    const lid = (p && typeof p === 'object') ? String(p.id || '') : '';
                    legacyOk = isFinite(lt) && lt > 2 && (!lid || !songId || lid === String(songId));
                } catch (_) { /* malformed legacy value → treat as missing */ }
                if (!legacyOk) {
                    localStorage.setItem('beatZen_lastPosition', JSON.stringify({
                        t: Number(v2.t),
                        d: Number(v2.d) > 0 ? Number(v2.d) : undefined,
                        id: songId != null ? String(songId) : ''
                    }));
                    window._bzRLog('healed legacy position from v2: ' + Number(v2.t).toFixed(1) + 's');
                }
        ...
        (function paintLastPlayedBar() {
            try {
                    ...
                    try {
                        ...
                        if (_audioEl) {
                            ...
                            if (_srcUnset) {
                                ...
                                // index.html ships preload="none": load() then fetches NOTHING, duration stays
                                // NaN, the saved position can never be seeked to and play() starts at 0:00.
                ...
                /* ── Reveal player bar (restore path) ──────────────────────
                   This block only runs when 'lastPlayedSong' exists in
                   localStorage, which is only ever written after a REAL
                   play (see the shouldPlay-gated save further down). So a
                   brand-new user (nothing ever played) never reaches here
                   and never sees the bar — but a returning user who has
                   played before gets it restored on every open, as expected. */
                (function _bzRevealPlayerRestore() {
                    var mp = document.getElementById('main-player');
                    if (mp) mp.classList.add('bz-player-active');
                    document.body.classList.add('bz-has-player');
                })();


                // Repair the legacy position record from the independent v2 record if needed
                try { if (saved.url) window._bzHealResumeStore(saved.url, songId); } catch (_) { }
        ...
        (function initHDAudioChain() {
            try {
                ...
                volumeBoost.gain.value = 0.8; // normal listening level
                ...
                // FIX (lock/unlock playback flicker): mobile browsers frequently
                // suspend the AudioContext itself when the screen locks or the tab
                // goes to the background — even though the underlying <audio>
                // element keeps reporting paused:false the whole time (the OS lets
                // the element itself keep running, it just freezes the Web Audio
                // graph). Because nothing else fires a fresh 'play' event in that
                // case, the listener above never re-runs, so every filter in this
                // chain (and therefore all sound) stays silent or glitches/crackles
                // once you unlock. Explicitly resume the context on every return to
                // the foreground instead of waiting on a 'play' event that won't come.
                function _bzResumeAudioContextIfNeeded() {
                    if (ctx.state === 'suspended' && !audioPlayer.paused) {
                        ctx.resume().catch(() => { });
                    }
                }
                document.addEventListener('visibilitychange', () => {
                    if (document.visibilityState === 'visible') _bzResumeAudioContextIfNeeded();
                });
                window.addEventListener('focus', _bzResumeAudioContextIfNeeded);
                // iOS bfcache restore (swipe back) also needs a check
                window.addEventListener('pageshow', (e) => { if (e.persisted) _bzResumeAudioContextIfNeeded(); });

                // Public API for a future frontend Bass / Vocal Clarity UI Percent-based
                const VOCAL_BODY_DB_AT_100 = 0.5;   // body-band boost at 100% clarity
                const CLARITY_DB_AT_100 = 1;      // presence-band boost at 100% clarity
                const DEFAULT_BASS_PERCENT = 190;    // punchy bass
        ...
        /* ── STAFF (OWNER + ADMIN) ACCESS ────────────────────────────────────
           FIX: admins were locked out of Home / Search / Playlists / Settings.
           Those nav items are premium-gated, and only the owner email ever
           received a premium grant (auth.js FREE_FOREVER_EMAILS) — the admin
           emails never did, so every admin except the owner was bounced to the
           Premium screen on every nav click. Owner + admin emails now bypass
           the premium gate (bzHasFullAccess below).

           These lists are declared HERE, not down beside the Admin Dashboard
           code, because _bzPremiumTick() (further below) calls
           bzApplyPremiumGating() at boot, and a const declared later in this
           function would still be in its temporal dead zone at that moment.

           Two access tiers — keep both lists in sync with firestore.rules
           (isAppOwner / isAdminEmail). Emails must be lowercase. */
        const BZ_OWNER_EMAILS = ['sairuthwik2002@gmail.com'];
        const BZ_ADMIN_EMAILS = [
            'dineshmummana777@gmail.com',
            'allaripilladolly@gmail.com'
        ];

        function bzCurrentEmail() {
            try {
        ...
        // Owner only
        function bzIsOwnerUser() {
            const email = bzCurrentEmail();
            return !!email && BZ_OWNER_EMAILS.indexOf(email) !== -1;
        }

        // Owner OR admin — gates the Admin Dashboard button and screens
        function bzIsAdminUser() {
            const email = bzCurrentEmail();
            return !!email && (BZ_OWNER_EMAILS.indexOf(email) !== -1 || BZ_ADMIN_EMAILS.indexOf(email) !== -1);
        }

        // Email used for the premium bypass. Normally the live Firebase user.
        // On a cold boot Firebase is still resolving (auth.currentUser is null
        // for ~100-300 ms) while bzNavGuard / the boot view-restore already run
        // optimistically off the cached session — so, exactly like they do, fall
        // back to the email auth.js cached for that session. This only applies
        // while Firebase is unresolved AND a cached session exists; once Firebase
        // answers, the live user is the only source of truth (sign-out clears it).
        function bzStaffAccessEmail() {
            const live = bzCurrentEmail();
            if (live) return live;
            if (window.bzIsAuthenticated === undefined && _bzLikelySignedIn()) {
                try {
                    return (localStorage.getItem('beatzen_displayUsername') || '').trim().toLowerCase();
                } catch (_) { }
            }
            return '';
        }

        function bzIsStaffUser() {
            const email = bzStaffAccessEmail();
            return !!email && (BZ_OWNER_EMAILS.indexOf(email) !== -1 || BZ_ADMIN_EMAILS.indexOf(email) !== -1);
        }
        window.bzIsStaffUser = bzIsStaffUser;

        // The single check every nav gate uses: an active premium plan OR
        // owner/admin. (bzIsPremiumUser stays a pure "has a plan" check so the
        // Premium screen and the Profile card still describe real plan status.)
        function bzHasFullAccess() {
            return bzIsPremiumUser() || bzIsStaffUser();
        }
        window.bzHasFullAccess = bzHasFullAccess;

        // Nav items that require an active premium subscription (or owner/admin
        // access — see bzHasFullAccess). Profile is
        // intentionally excluded, and the Premium screen itself was never a
        // gated nav item — a locked-out user must always be able to reach
        // Premium (now opened from the Profile page) and their own Profile.
        const BZ_PREMIUM_LOCKED_LINK_IDS = ['home-link', 'search-link', 'playlists-link', 'settings-link'];
        const BZ_PREMIUM_LOCKED_VIEWS = ['home', 'search', 'playlists', 'settings'];

        // Keeps the Profile page's Premium entry card in sync with the
        // user's live premium status (flips from "Premium starts from ₹9 — Unlock now" to
        // an active-plan blurb, and back again the instant a subscription
        // expires). Safe to call even when the Profile page isn't visible.
        function bzRefreshProfilePremiumCard() {
            const sub = document.getElementById('bz-profile-premium-sub');
            if (!sub) return;
            sub.textContent = bzIsPremiumUser()
                ? 'All features unlocked — manage your plan here'
                : (bzIsStaffUser() ? 'Admin access — all features unlocked' : 'Premium starts from ₹9 — Unlock now');
        }
        window.bzRefreshProfilePremiumCard = bzRefreshProfilePremiumCard;
        ...
        function bzApplyPremiumGating() {
            const unlocked = bzHasFullAccess();
        ...
        let _bzPremiumSelectedPlan = { id: 'month', hours: 720, amount: 9 }; // matches the pre-selected card
        ...
        /* ─────────────────────────────────────────────────────────────────
           ...
           (There used to be a third automatic check — that the paid amount
           in the screenshot matched the selected plan's price — but OCR
           amount-matching produced enough false rejections on legitimate
           payments that it's been dropped. The amount the user claims is
           still recorded on the request doc for the admin to review.)
        ...
        async function bzSubmitPremiumRequest() {
            ...
            try {
                ...
                // The plan (and its price) the user selected in the UI —
                // amount is no longer auto-checked against the screenshot,
                // so this is exactly what the admin dashboard will show.
        ...
        function _bzPremiumPlanLabel(id) {
            return id === 'month' ? '1 Month' : (id === 'quarter' ? '3 Months' : (id === 'year' ? 'Yearly' : (id === 'signup_bonus' ? '24-Hour Free Trial' : 'Premium')));
        }
        ...
        // Builds a plan-aware UPI QR (pa/am/tn encoded in the link) so the
        // code scanned always matches the selected plan's amount.
        function _bzUpdatePremiumQr() {
            const imgEl = document.getElementById('bz-premium-qr-img');
            const upiIdEl = document.getElementById('bz-premium-upi-id');
            if (!imgEl || !upiIdEl) return;
            const upiId = upiIdEl.textContent.trim();
            const amount = (_bzPremiumSelectedPlan && _bzPremiumSelectedPlan.amount) || 0;
            const planId = (_bzPremiumSelectedPlan && _bzPremiumSelectedPlan.id) || 'premium';
            const upiUri = 'upi://pay?pa=' + encodeURIComponent(upiId) +
                '&pn=' + encodeURIComponent('Beat Zen') +
                '&am=' + encodeURIComponent(amount) +
                '&cu=INR&tn=' + encodeURIComponent('Beat Zen Premium - ' + planId);
            // Hosted QR image API — used as the fallback (and as the
            // primary source if the QRCode library never finished loading)
            // so the code always renders even if the CDN script was
            // blocked, slow, or failed.
            ...
            if (typeof QRCode !== 'undefined' && typeof QRCode.toDataURL === 'function') {
                QRCode.toDataURL(upiUri, { width: 300, margin: 1 }, function (err, url) {
                    imgEl.src = (!err && url) ? url : fallbackUrl;
                });
            } else {
                imgEl.src = fallbackUrl;
            }
            // Belt-and-suspenders: if whatever we set still fails to load
            // (e.g. a bad data URL), drop back to the hosted API image.
        ...
        function bzInitPremiumUI() {
            ...
            const amountPlanEl = document.getElementById('bz-premium-amount-plan');
            document.querySelectorAll('#bz-premium-plans-grid .bz-premium-plan-card').forEach(function (card) {
                card.addEventListener('click', function () {
                    ...
                    const planNameEl = card.querySelector('.bz-premium-plan-name');
                    if (amountPlanEl && planNameEl) amountPlanEl.textContent = 'Beat Zen Premium · ' + planNameEl.textContent.trim();
                    _bzUpdatePremiumQr();
                });
            });
            _bzUpdatePremiumQr();
            ...
            const upiIdEl = document.getElementById('bz-premium-upi-id');
            if (copyBtn && upiIdEl) {
                const _flashCopied = function () {
                    copyBtn.classList.add('bz-copied');
                    copyBtn.innerHTML = '<i class="fas fa-check"></i> Copied';
                    clearTimeout(copyBtn._bzT);
                    copyBtn._bzT = setTimeout(function () {
                        copyBtn.classList.remove('bz-copied');
            ...
            // Download the QR as a PNG. It is redrawn on a white canvas with a
            // quiet-zone border so the saved file scans easily from a gallery;
            // if the canvas is blocked (cross-origin fallback image) the raw
            // image is saved instead.
            const qrDownloadBtn = document.getElementById('bz-premium-qr-download-btn');
            if (qrDownloadBtn && qrImgEl) {
                qrDownloadBtn.addEventListener('click', async function () {
                    try {
                        let blob = null;
                        try {
                            const img = new Image();
                            img.crossOrigin = 'anonymous';
                            await new Promise(function (res, rej) { img.onload = res; img.onerror = rej; img.src = qrImgEl.src; });
                            ...
                            cx.drawImage(img, pad, pad, size - pad * 2, size - pad * 2);
                        ...
                        console.error('Beat Zen: QR download failed', e);
                        showToast("Couldn't download the QR image.");
                    }
                });
            }
            ...
            // "Upgrade to Premium" — while the free trial (or an existing
            // paid plan) is still active, this drops the user straight into
            // the plan-picker/payment view instead of just dismissing the
            // status screen, so they can buy before the trial runs out.
        ...
        window.playSong = async function (index, shouldPlay = true) {
            ...
            if (shouldPlay) {
                ...
                localStorage.removeItem('beatZen_resume_v2'); // a deliberately (re)started song has no resume point
            ...
            // Reveal player bar — on a real play, and also as a safety net
            // during session restore (shouldPlay=false) in case the earlier
            // fast-path restore (paintLastPlayedBar) didn't run for some
            // reason. Brand-new users never reach this function with a saved
            // song at all, so they're unaffected.
            ...
            audioPlayer._bzPendingResume = null; // a new load supersedes any unfinished resume-seek
            audioPlayer._bzArmedResume = null;
            ...
            if (!_instantRestoreHit) {
                // Restore loads (shouldPlay=false) must fetch metadata or the saved position
                // can never be applied while the element is idle (see preload="none" note above).
                if (!shouldPlay) audioPlayer.preload = 'metadata';
            ...
            if (shouldPlay) {
                ...
                // applySavedTime now returns a promise that resolves once the
                // seek has actually landed (or been given up on) — kept on
                // window so togglePlayback() can wait on the SAME in-flight
                // restore instead of racing it.
                try { window._bzHealResumeStore(song.url, songIdStr); } catch (_) { }
                window._bzRestorePromise = window.applySavedTime ? window.applySavedTime() : Promise.resolve();
                // FIX Bug 6: if the user hit play while restore was still in progress.
                // Previously this called play() right away, which could start
                // playback from 0 before the seek above had actually landed —
                // now it waits for the same restore promise first.
                if (window._bzAutoPlayAfterRestore) {
                    window._bzAutoPlayAfterRestore = false;
                    Promise.race([Promise.resolve(window._bzRestorePromise), new Promise(r => setTimeout(r, 1000))]).then(() => {
                        if (window._bzArmPendingResume) window._bzArmPendingResume();
                        audioPlayer.play().then(() => {
                            updatePlayPauseIcon(); updateDynamicTitle();
        ...
        function togglePlayback() {
            ...
            if (audioPlayer.paused) {
                const _startPlayback = () => {
                    // If the saved position never landed (idle element, slow network, iOS quirks),
                    // make sure THIS play() starts from it instead of 0:00.
                    if (window._bzArmPendingResume) window._bzArmPendingResume();
                    return audioPlayer.play().then(() => { updatePlayPauseIcon(); updateDynamicTitle(); if ('mediaSession' in navigator) navigator.mediaSession.playbackState = "playing"; }).catch(() => { });
                };
                if (!audioPlayer._restoreApplied && window._bzRestorePromise) {
                    // A saved position hasn't finished restoring yet (still waiting on
                    // the audio to buffer enough to seek). Wait for that SAME in-flight
                    // restore instead of playing now — otherwise playback starts from 0
                    // and the seek either never gets a chance to correct it, or does so
                    // as a jarring jump mid-playback.
                    // Bounded wait: if the seek is still unresolved after 1s, play anyway —
                    // the resume guard below guarantees the saved position is applied.
                    Promise.race([Promise.resolve(window._bzRestorePromise), new Promise(r => setTimeout(r, 1000))]).then(_startPlayback);
                } else {
                    _startPlayback();
                }
            } else {
        ...
        /* BZ-RESUME-GUARD:BEGIN */
        /* ═════════════════════════ RESUME GUARD v2 ═════════════════════════
           Goal: pressing play on a paused / freshly-restored song ALWAYS continues from where
           it stopped, whichever button, lock-screen control or keyboard shortcut triggered it.

           1. currentTime write guard – a write that is NOT (within 2s of) the pending resume target
              is a deliberate user seek (scrub, lyric tap, prev = 0 …) and cancels the auto-resume.
           2. play() interceptor      – every play() call on the element first applies a pending resume.
           3. Arm + verify            – apply the target now (Chrome/Firefox keep it as the default start
              position even before metadata exists) and again on loadedmetadata / canplay / playing
              (iOS Safari drops early writes); 1.5s after playback starts check it really stuck and retry.
           4. Independent recorder    – beatZen_resume_v2 {u:url, t, d} written ≤1×/s while playing and on
              pause / hide / pagehide / freeze, cleared on 'ended' and on a deliberate new play. Keyed by
              the audio URL, so it survives index/album changes and the legacy key being wiped. */
        (function bzResumeGuard() {
            const RK = 'beatZen_resume_v2';
            const rlog = window._bzRLog || function () { };

            // 1 ── currentTime write guard
            (function () {
                const desc = (typeof HTMLMediaElement !== 'undefined')
                    ? Object.getOwnPropertyDescriptor(HTMLMediaElement.prototype, 'currentTime') : null;
                ...
                Object.defineProperty(audioPlayer, 'currentTime', {
                    ...
                    set(v) {
                        const n = Number(v);
                        if (this._bzPendingResume != null && !(Math.abs(n - this._bzPendingResume) <= 2)) this._bzPendingResume = null;
                        if (this._bzArmedResume != null && !(Math.abs(n - this._bzArmedResume) <= 2)) this._bzArmedResume = null;
            ...
            // 3 ── arm + verify
            window._bzArmPendingResume = function () {
                const target = audioPlayer._bzPendingResume;
                audioPlayer._bzPendingResume = null;               // one-shot
                if (!isFinite(target) || target <= 2) return;
                const armedSrc = audioPlayer.src;
                audioPlayer._bzArmedResume = target;
                let finished = false, overallTimer = null, verifyTimer = null, attempts = 0;

                const goal = () => {
                    const d = audioPlayer.duration;
                    return (isFinite(d) && d > 1) ? Math.min(target, d - 1) : target;
                };
                const alive = () => !finished && audioPlayer._bzArmedResume === target && audioPlayer.src === armedSrc;

                function finish() {
                    if (finished) return;
                    finished = true;
                    clearTimeout(overallTimer); clearTimeout(verifyTimer);
                    audioPlayer.removeEventListener('loadedmetadata', apply);
                    audioPlayer.removeEventListener('canplay', apply);
                    audioPlayer.removeEventListener('playing', onPlaying);
                    if (audioPlayer._bzArmedResume === target) audioPlayer._bzArmedResume = null;
                }
                function apply() {
                    if (!alive()) { finish(); return; }              // another song / a user seek took over
                    const g = goal();
                    if (Math.abs(audioPlayer.currentTime - g) > 2) {
                        try { audioPlayer.currentTime = g; } catch (_) { /* not seekable yet — next event retries */ }
                    }
                }
                function onPlaying() {
                    audioPlayer.removeEventListener('playing', onPlaying);
                    apply();
                    verifyTimer = setTimeout(function verify() {
                        if (!alive()) { finish(); return; }
                        const g = goal(), ct = audioPlayer.currentTime;
                        if (ct >= g - 3) { rlog('resume OK at ' + ct.toFixed(1) + 's (target ' + g.toFixed(1) + 's)'); finish(); return; }
                        if (++attempts <= 2) {
                            rlog('resume retry ' + attempts + ': at ' + ct.toFixed(1) + 's, wanted ' + g.toFixed(1) + 's');
                            try { audioPlayer.currentTime = g; } catch (_) { /* retry on next tick */ }
                            verifyTimer = setTimeout(verify, 1500);
                            return;
                        }
                        const sk = audioPlayer.seekable, end = (sk && sk.length) ? sk.end(sk.length - 1) : 0;
                        rlog('resume FAILED: at ' + ct.toFixed(1) + 's wanted ' + g.toFixed(1) + 's | seekable ranges=' + (sk ? sk.length : 0)
                            + ' end=' + end.toFixed(1) + 's | readyState=' + audioPlayer.readyState + ' networkState=' + audioPlayer.networkState
                            + (end < g - 1 ? ' | audio host probably lacks HTTP Range support' : ''));
                        if (typeof showToast === 'function') showToast("Couldn't jump back to where you left off on this track.");
                        finish();
                    }, 1500);
                }

                audioPlayer.addEventListener('loadedmetadata', apply);
                audioPlayer.addEventListener('canplay', apply);
                audioPlayer.addEventListener('playing', onPlaying);
                overallTimer = setTimeout(finish, 15000);
                rlog('resume armed: target ' + target.toFixed(1) + 's readyState=' + audioPlayer.readyState + ' duration=' + audioPlayer.duration);
                apply();
            };

            // 2 ── play() interceptor (covers main button, mini-player, lock screen, keyboard, fallbacks…)
            if (!audioPlayer._bzPlayWrapped) {
                const nativePlay = audioPlayer.play;
                audioPlayer.play = function () {
                    if (this._bzPendingResume != null) { try { window._bzArmPendingResume(); } catch (_) { /* never block play() */ } }
                    return nativePlay.apply(this, arguments);
                };
                audioPlayer._bzPlayWrapped = true;
            }

            // 4 ── independent recorder
            let lastWrite = 0;
            function record(force) {
                try {
                    ...
                    if (audioPlayer.ended || !isFinite(cur) || cur <= 2) return;
                    if (audioPlayer._bzPendingResume != null) return;                       // seek not applied yet
                    if (audioPlayer._bzArmedResume != null && cur < audioPlayer._bzArmedResume - 3) return; // still catching up
                    const now = Date.now();
                    if (!force && now - lastWrite < 1000) return;
                    lastWrite = now;
                    localStorage.setItem(RK, JSON.stringify({ u: audioPlayer.src, t: cur, d: (isFinite(dur) && dur > 0) ? dur : 0, at: now }));
                } catch (_) { /* storage full/unavailable — next tick retries */ }
            }
            audioPlayer.addEventListener('timeupdate', () => record(false));
            audioPlayer.addEventListener('pause', () => record(true));
            audioPlayer.addEventListener('ended', () => { try { localStorage.removeItem(RK); } catch (_) { } });
            document.addEventListener('visibilitychange', () => { if (document.hidden) record(true); });
            document.addEventListener('freeze', () => record(true));
        ...
        function syncProgressBar() {
            ...
            // Resume still pending while paused: keep showing the saved position instead of 0:00
            if (audioPlayer.paused && audioPlayer._bzPendingResume != null) return;
            const cur = audioPlayer.currentTime, dur = audioPlayer.duration;
        ...
        function attachScrubHandlers(barEl, fillEl, timeEl) {
            ...
            const handleScrub = (e) => {
                ...
                audioPlayer._bzPendingResume = null; // user chose a position — don't override it
                audioPlayer._bzArmedResume = null;
        ...
        function displayHome(isBack = false, targetYear = null) {
            ...
            if (!yearSectionsContainer.innerHTML.trim() || yearSectionsContainer.dataset.builtFor !== window._bzDataVersion) {
                /* Build the real cards straight away — the album data is
                   already in memory, so there's nothing to "load" here.
                   Building synchronously (instead of showing a skeleton
                   and swapping in real cards a frame later) removes the
                   blank flash between clicking Home and seeing content. */
                yearSectionsContainer.innerHTML = '';
                const frag = document.createDocumentFragment();
                allYears.forEach(year => {
                    const sec = document.createElement('div');
                    sec.className = 'year-section'; sec.id = `year-sec-${year}`;
        ...
        function displayPlaylists(isBack = false) {
            /* Build the real content immediately — window.masterPool is
               already in memory, so there's nothing to "load" here.
               Building synchronously (instead of showing a skeleton and
               swapping in real cards a frame later) removes the blank/
               skeleton flash between clicking Playlists and seeing content. */
            playlistsContainer.innerHTML = '';

            /* Section heading: "Your Playlists" — built only once the
               user actually has one. A heading with nothing under it
               reads as a broken section, so when the pool holds no
               playlists the whole block is skipped and Smart Playlists
               below starts the page instead. The nav pill follows
               automatically: updateNavPillVisibility() hides any pill
               whose section is missing or empty. */
            const userPlaylists = (window.masterPool || []).filter(p => p && p.type === 'Playlist');

            if (userPlaylists.length) {
                const sec = document.createElement('div');
                sec.className = 'bzp-section dp-section';
        ...
        // Updates page — opened from within Settings, same pattern as Premium from Profile.
        ...
        // BZ_OWNER_EMAILS / BZ_ADMIN_EMAILS / bzCurrentEmail / bzIsOwnerUser /
        // bzIsAdminUser now live up in the PREMIUM section ("STAFF ACCESS"),
        // because the premium gate has to know about them at boot.
        ...
        // Live listener on a user's beatzen_users/{uid}/devices subcollection,
        // active only while that user's detail pane is open.
        let _bzAdminDevicesUnsub = null;
        ...
        function _bzAdminStopDevicesListener() {
            if (_bzAdminDevicesUnsub) { _bzAdminDevicesUnsub(); _bzAdminDevicesUnsub = null; }
        }
        ...
        // A request doc's status stays 'active' in Firestore forever once
        // written — nothing flips it when the plan's hours simply run out.
        // _bzRenderAdminPremiumList() re-derives "still active" vs "expired"
        // from Date.now() each time it runs, but it only *runs* on a
        // Firestore change. This timer re-renders on a plain clock tick too,
        // so a card flips from Active to Expired on its own the moment the
        // plan's time is up, even if nothing else about the doc changes.
        let _bzAdminPremiumRefreshTimer = null;
        function _bzAdminStopPremiumListener() {
            if (_bzAdminPremiumUnsub) { _bzAdminPremiumUnsub(); _bzAdminPremiumUnsub = null; }
            if (_bzAdminPremiumRefreshTimer) { clearInterval(_bzAdminPremiumRefreshTimer); _bzAdminPremiumRefreshTimer = null; }
        }
        ...
        function _bzAdminDeclinePremium(uid, btn) {
            ...
            const reasons = [
                ...
                { value: 'Payment not verified inappropriate screenshot. Please upload a valid screenshot.', label: 'Screenshot unclear / unreadable' },
        ...
        // Permanently removes a premium-request entry from the Admin
        // history list — the beatzen_premium_requests/{uid} doc itself is
        // deleted (not just its status flipped), so it's gone from both
        // this list and the database. If the entry is still a live ACTIVE
        // (non-expired) grant, this revokes premium on beatzen_users first
        // — otherwise the record of the grant would vanish while the
        // user's access silently continued.
        async function _bzAdminDeleteHistoryEntry(uid, btn, onDone) {
            if (!uid || typeof db === 'undefined') return;
            if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i>'; }
            try {
                const entry = _bzAdminPremiumData.find(function (x) { return x.id === uid; });
                const isActiveLive = !!(entry && entry.status === 'active' && (!entry.expiresAt || Date.now() < entry.expiresAt));
                if (isActiveLive) {
                    await db.collection('beatzen_users').doc(uid).set({
                        premium: false,
                        premiumExpiresAt: 0
                ...
                await db.collection('beatzen_premium_requests').doc(uid).delete();
                showToast('✓ Deleted from history');
                if (typeof onDone === 'function') onDone();
            } catch (e) {
                console.error('Beat Zen: delete history entry failed', e);
                showToast("Couldn't delete — please try again.");
                if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-trash"></i> Delete'; }
            }
        }

        // Confirmation gate in front of _bzAdminDeleteHistoryEntry —
        // permanent + irreversible, so it always asks first.
        function bzAdminConfirmDeleteHistoryEntry(uid, name, btn, onDone) {
            window.bzConfirm(
                'danger',
                'Delete from history?',
                (name ? _bzEscapeHTML(name) : 'This entry') + "'s premium request record will be permanently deleted from the database. This can't be undone.",
                function () { _bzAdminDeleteHistoryEntry(uid, btn, onDone); },
                'Delete',
                'Cancel'
            );
        }
        ...
        function _bzRenderAdminPremiumList() {
            ...
            listEl.innerHTML = _bzAdminPremiumData.map(function (d) {
                ...
                const planLabel = d.plan === 'month' ? '1 Month' : (d.plan === 'quarter' ? '3 Months' : (d.plan === 'year' ? 'Yearly' : _bzEscapeHTML(d.plan || '')));
                // A doc can sit at status:'active' forever once its expiresAt
                // has passed — nothing flips it automatically. So "still
                // active" here means the status AND the plan's own
                // hours-based expiry both say so, per-account, not just the
                // stored status string.
                const isExpired = d.status === 'active' && !!d.expiresAt && Date.now() >= d.expiresAt;
                const statusBadge = isExpired
                    ? '<span class="bz-admin-premium-status bz-admin-premium-status--expired">Expired</span>'
                    : d.status === 'active'
                ...
                const deleteBtnHtml = `<button type="button" class="stg-action-btn stg-btn--danger bz-admin-premium-delete" data-uid="${d.id}"><i class="fas fa-trash"></i> Delete</button>`;
                const actionsHtml = d.status === 'pending'
                    ...
                    : (d.status === 'active' && !isExpired)
                        ...
                        : `<div class="bz-admin-premium-actions">${deleteBtnHtml}</div>`;
                return `
            ...
            listEl.querySelectorAll('.bz-admin-premium-delete').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    const uid = btn.getAttribute('data-uid');
                    const entry = _bzAdminPremiumData.find(function (x) { return x.id === uid; });
                    bzAdminConfirmDeleteHistoryEntry(uid, entry && (entry.name || entry.email), btn);
                });
            });
        }
        ...
        function _bzAdminEnsurePremiumListener() {
            ...
            if (!_bzAdminPremiumRefreshTimer) {
                _bzAdminPremiumRefreshTimer = setInterval(_bzRenderAdminPremiumList, 60000);
            }
        }
                ...
                _bzAdminUserListUnsub = db.collection('beatzen_users').onSnapshot(function (snap) {
                    ...
                    snap.forEach(function (doc) {
                        ...
                        _bzAdminUsersData.push({
                            ...
                            activeDeviceId: d.activeDeviceId || null,
                            activeDeviceName: d.activeDeviceName || null
                        });
                    });
        ...
        // Devices — one row per browser/device that has ever signed into this
        // account, sourced from beatzen_users/{uid}/devices (written by
        // bzRecordDeviceLogin() in auth.js on every successful sign-in).
        function _bzDeviceIcon(deviceType) {
            if (deviceType === 'mobile') return 'fa-mobile-alt';
            if (deviceType === 'tablet') return 'fa-tablet-alt';
            return 'fa-laptop';
        }

        function _bzFmtDeviceTime(ts) {
            if (!ts) return '—';
            return typeof fmtTimestamp === 'function' ? fmtTimestamp(ts) : (ts.toDate ? ts.toDate().toLocaleString() : '—');
        }

        // Human "last seen" phrasing for a device that isn't the active one
        // right now — "Just now" / "5m ago" / "3h ago" / "2d ago", falling
        // back to a month/year figure once it's older.
        function _bzRelativeTime(ts) {
            const ms = (ts && ts.toDate) ? ts.toDate().getTime() : (typeof ts === 'number' ? ts : null);
            if (!ms) return 'Last seen —';
        ...
        // Some devices end up with more than one row for what is really the
        // same browser/device — e.g. if the stored per-device id wasn't
        // stable across sign-ins and a fresh doc got created instead of the
        // existing one being updated (this is what produced the duplicate
        // "Chrome on Windows" rows). Collapse rows that share a device
        // name + type into a single row before rendering: sign-in counts
        // are summed, the earliest firstLoginAt and latest lastLoginAt are
        // kept, and if any of the merged rows is the currently-active
        // device that id wins so "Active now" still lands correctly.
        function _bzDedupeDevices(devices, activeDeviceId) {
            if (!Array.isArray(devices)) return [];
            const byKey = new Map();
            devices.forEach(function (dev) {
                const key = (dev.deviceName || 'unknown').trim().toLowerCase() + '|' + (dev.deviceType || '');
                const existing = byKey.get(key);
                if (!existing) {
                    byKey.set(key, Object.assign({}, dev));
                    return;
                }
                existing.loginCount = (existing.loginCount || 0) + (dev.loginCount || 0);
                if ((dev._lastLoginMs || 0) > (existing._lastLoginMs || 0)) {
                    existing.lastLoginAt = dev.lastLoginAt;
                    existing._lastLoginMs = dev._lastLoginMs;
                }
                const existingFirstMs = (existing.firstLoginAt && existing.firstLoginAt.toDate) ? existing.firstLoginAt.toDate().getTime() : Infinity;
                const devFirstMs = (dev.firstLoginAt && dev.firstLoginAt.toDate) ? dev.firstLoginAt.toDate().getTime() : Infinity;
                if (devFirstMs < existingFirstMs) existing.firstLoginAt = dev.firstLoginAt;
                // Keep whichever underlying doc id is the active one so the
                // "Active now" badge still lands on the merged row.
                if (activeDeviceId && dev.id === activeDeviceId) existing.id = dev.id;
            });
            return Array.from(byKey.values()).sort(function (a, b) { return (b._lastLoginMs || 0) - (a._lastLoginMs || 0); });
        }

        // activeDeviceId (from the user's beatzen_users doc — see
        // bzRecordDeviceLogin() in auth.js) marks which single device is
        // currently allowed to be signed in; every other device in this list
        // has already been signed out automatically the moment it lost that
        // status, but stays listed here as sign-in history.
        function _bzRenderDevicesSection(devices, activeDeviceId) {
            const deduped = _bzDedupeDevices(devices, activeDeviceId);
            if (deduped.length === 0) {
                return '<p style="opacity:.5; font-size:.85rem;">No devices recorded yet.</p>';
            }
            const shown = deduped.slice(0, 20).map(function (dev) {
                const count = dev.loginCount || 0;
                const isActive = !!activeDeviceId && dev.id === activeDeviceId;
                const activeBadge = isActive
                    ? '<span style="display:inline-block; margin-left:8px; padding:2px 8px; border-radius:999px; font-size:.68rem; font-weight:700; background:rgba(29,185,84,0.15); color:#3ddc71; vertical-align:middle;">Active now</span>'
                    : '';
                // Active device: highlighted via the badge above + the
                // "Active now" status below. Everything else: show how long
                // ago it was last signed into instead of a raw sign-in
                // count, so it's clear at a glance which device is current
                // vs. stale history.
                const statusHtml = isActive
                    ? '<span style="font-size:.75rem; font-weight:700; color:#3ddc71; white-space:nowrap; flex-shrink:0;">Active now</span>'
                    : `<span style="opacity:.55; font-size:.75rem; white-space:nowrap; flex-shrink:0;">${_bzEscapeHTML(_bzRelativeTime(dev.lastLoginAt))}</span>`;
                return `<div style="display:flex; justify-content:space-between; align-items:center; gap:12px; padding:10px; margin-bottom:8px; background:rgba(255,255,255,0.04); border-radius:10px; ${isActive ? 'box-shadow:inset 0 0 0 1px rgba(29,185,84,0.35);' : ''}">
                    <div style="display:flex; align-items:center; gap:10px; min-width:0;">
                        <i class="fas ${_bzDeviceIcon(dev.deviceType)}" style="opacity:.6; font-size:1rem; flex-shrink:0; width:18px; text-align:center;"></i>
                        <div style="min-width:0;">
                            <div style="font-weight:700; font-size:.88rem; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${_bzEscapeHTML(dev.deviceName || 'Unknown device')}${activeBadge}</div>
                            <div style="opacity:.5; font-size:.72rem; margin-top:2px;">${count} sign-in${count === 1 ? '' : 's'}</div>
                        </div>
                    </div>
                    ${statusHtml}
                </div>`;
            }).join('');
            return shown + (deduped.length > 20 ? `<p style="opacity:.5; font-size:.76rem; margin-top:6px;">+ ${deduped.length - 20} more device${deduped.length - 20 === 1 ? '' : 's'}</p>` : '');
        }
        ...
        // devices (from the beatzen_users/{uid}/devices listener in
        // bzShowAdminUserDetail) lists every device that has signed in.
        function _bzRenderUserDetail(d, userMeta, devices) {
            d = d || {};
            ...
            const devicesHtml = _bzRenderDevicesSection(devices, userMeta && userMeta.activeDeviceId);

            return _bzSection('Premium', 'fa-crown', premiumRows + premiumActionHtml)
                + _bzSection('Profile', 'fa-user', profileRows)
                + _bzSection('Devices', 'fa-mobile-alt', devicesHtml)
        ...
        function bzShowAdminUserDetail(uid) {
            ...
            function _renderDetailPane() {
                ...
                detailEl.innerHTML = _bzAdminBackToListButton()
                    ...
                    + _bzRenderUserDetail(_lastSyncData, userMeta, _lastDevicesData);
                const backBtn = document.getElementById('bz-admin-back-to-list-btn');
                if (backBtn) backBtn.addEventListener('click', function () { _bzAdminStopUserDetailListener(); _bzAdminStopDevicesListener(); bzShowAdminUserList(); });
            ...
            // Live devices list for this user — populates the "Devices" section
            _bzAdminStopDevicesListener();
            _bzAdminDevicesUnsub = db.collection('beatzen_users').doc(uid).collection('devices').onSnapshot(function (snap) {
                const rows = [];
                snap.forEach(function (doc) {
                    const dd = doc.data() || {};
                    rows.push({
                        id: doc.id,
                        deviceName: dd.deviceName || 'Unknown device',
                        deviceType: dd.deviceType || 'desktop',
                        loginCount: typeof dd.loginCount === 'number' ? dd.loginCount : 0,
                        firstLoginAt: dd.firstLoginAt || null,
                        lastLoginAt: dd.lastLoginAt || null,
                        _lastLoginMs: (dd.lastLoginAt && dd.lastLoginAt.toDate) ? dd.lastLoginAt.toDate().getTime() : 0
                    });
                });
                rows.sort(function (a, b) { return b._lastLoginMs - a._lastLoginMs; });
                _lastDevicesData = rows;
                _renderDetailPane();
            }, function (err) {
                console.warn('[BeatZen Admin] devices listener failed:', err && err.message);
                _lastDevicesData = [];
                _renderDetailPane();
            });
        }
        ...
        <div class="songs-list"></div>`;
            ...
            requestAnimationFrame(() => {
                ...
                album.songs.forEach((song, i) => {
                    ...
                    item.onclick = () => {
                        // Tapping the row of the song that is ALREADY loaded and paused mid-way resumes it
                        // instead of reloading it from 0:00.
                        if (window.playingAlbum && window.currentAlbum
                            && String(window.playingAlbum.id) === String(window.currentAlbum.id)
                            && window.currentSongIndex === i
                            && String(window.playingAlbum.songs?.[i]?.id) === String(song.id)
                            && audioPlayer.paused && audioPlayer.src && audioPlayer.src !== window.location.href
                            && (audioPlayer.currentTime > 2 || audioPlayer._bzPendingResume != null)) {
                            window._highlightActive = true;
            ...
            albumMainContent.querySelector('.play-album-btn').onclick = () => {
                ...
                if (window.playingAlbum && String(window.playingAlbum.id) === String(album.id)) {
                    window.togglePlayback();
                } else {
        ...
        document.getElementById('confirm-delete-btn')?.addEventListener('click', () => {
            ...
            if (String(window.playingAlbum?.id) === String(playlistToDelete)) {
                stopAndReset();
                window.playingAlbum = null;
                // Nothing is loaded any more, so the mini-player goes away (it
                // only exists while a song is loaded) rather than showing an
                // empty placeholder over the nav. The saved "last played" song
                // pointed into the deleted playlist, so forget it too — it
                // would otherwise bring the bar back on the next launch.
                document.getElementById('main-player')?.classList.remove('bz-player-active');
                document.body.classList.remove('bz-has-player');
                const titleEl = document.getElementById('player-song-title');
                const artistEl = document.getElementById('player-song-artist');
                const coverEl = document.getElementById('player-album-cover');
                ...
                try { localStorage.removeItem('lastPlayedSong'); } catch (_) { }
        ...
        if (homeLink) homeLink.onclick = (e) => {
            e.preventDefault();
            bzNavGuard(() => {
                if (!bzHasFullAccess()) { displayPremium(); return; }
                displayHome();
            });
        };
        ...
        // Premium lives on the Profile page now — this card is the way in,
        // and it's never gated by itself (it IS the way out for a locked-out
        // user, same as before, just reached from Profile instead of a
        // top-level nav tab).
        const profilePremiumBtn = document.getElementById('bz-profile-premium-card');
        if (profilePremiumBtn) profilePremiumBtn.onclick = (e) => {
            e.preventDefault();
            bzNavGuard(() => {
                const gate = document.getElementById('bz-auth-gate');
                if (gate) gate.classList.remove('bz-gate-visible');
        ...
        // Premium screen's own back button returns to Profile, its entry point.
        const bzPremiumBackBtn = document.getElementById('bz-premium-back-btn');
        if (bzPremiumBackBtn) bzPremiumBackBtn.onclick = () => {
            if (window._bzSpaNavDepth > 0) {
                history.back();
            } else {
                displayProfile();
        ...
        // Updates row inside Settings — opens the Updates page.
        const bzUpdatesOpenBtn = document.getElementById('bz-updates-open-btn');
        if (bzUpdatesOpenBtn) bzUpdatesOpenBtn.onclick = (e) => {
            e.preventDefault();
            bzNavGuard(() => { displayUpdates(); });
        };
        // Updates screen's own back button returns to Settings, its entry point.
        const bzUpdatesBackBtn = document.getElementById('bz-updates-back-btn');
        if (bzUpdatesBackBtn) bzUpdatesBackBtn.onclick = () => {
            if (window._bzSpaNavDepth > 0) {
                history.back();
            } else {
                displaySettings();
        ...
        // POSITION RESTORE
        // Returns a Promise that resolves once the restore attempt is fully
        // settled (seek landed, gave up, or wasn't needed) — NOT just once
        // this function has been called. Callers that want to start playback
        // right after a restore should wait on this promise instead of
        // calling audioPlayer.play() immediately, otherwise playback can
        // start from 0 while the seek is still pending (the "click play and
        // it restarts from the beginning" bug).
        window.applySavedTime = function () {
            return new Promise(function (resolve) {
                ...
                if (audioPlayer._restoreApplied) { resolve(); return; }

                // Remember the target until a seek is CONFIRMED (cleared in confirmSeek below).
                // The resume guard uses this as a backstop if every seek attempt below gives up.
                audioPlayer._bzPendingResume = saved;
                if (window._bzRLog) window._bzRLog('restore: target ' + saved.toFixed(1) + 's readyState=' + audioPlayer.readyState + ' duration=' + audioPlayer.duration + ' preload=' + audioPlayer.preload);
                ...
                function doSeek() {
                    ...
                    // Not risky means readyState was already >= 2, so this assignment
                    // takes effect right away — safe to let a caller start playback
                    // now instead of waiting on the confirmSeek dance below, which
                    // exists mainly to correct the rarer "risky" seeks.
                    if (!wasRisky) resolve();
                    ...
                    function confirmSeek() {
                        if (Math.abs(audioPlayer.currentTime - safe) > 2 && retries < MAX_RETRIES) {
                            ...
                            if (wasRisky && !_secondChanceArmed && !audioPlayer._restoreApplied) {
                                ...
                                if (window._bzRLog) window._bzRLog('restore: seek did not land — backstop will apply it on play');
                            }
                        } else {
                            // Seek confirmed — update UI to match actual position
                            audioPlayer._restoreApplied = true;
                            audioPlayer._bzPendingResume = null; // landed — nothing left to enforce
                            if (window._bzRLog) window._bzRLog('restore: seek landed at ' + audioPlayer.currentTime.toFixed(1) + 's');
                            resolve();
                ...
                audioPlayer._restoreTimeout = setTimeout(() => {
                    ...
                    if (isFinite(audioPlayer.duration) && audioPlayer.duration > 0) {
                        ...
                        audioPlayer._restoreApplied = true;
                        if (window._bzRLog) window._bzRLog('restore: no duration yet — backstop will apply it on play');
                        resolve();
                        if (audioPlayer._restoreCPHandler) {
                            audioPlayer.removeEventListener('canplay', audioPlayer._restoreCPHandler);
                            audioPlayer.removeEventListener('loadedmetadata', audioPlayer._restoreCPHandler);
        ...
        function handleDeepLinking(navFromOverride) {
            ...
            if (!bzHasFullAccess() && hash !== '#profile' && hash !== '#premium') {
                displayPremium(true);
                return;
            }
        ...
        setTimeout(() => {
            ...
            bzNavGuard(function _restoreBootView() {
                ...
                if (!bzHasFullAccess() && !_wantsProfile && !_wantsPremium) {
                    displayPremium(true);
                    return;
                }
...
(function () {
    ...
    function _tryRestoreSession(attemptsLeft) {
        ...
        if (!savedSong) {
            state.restored = true;
            state.restoring = false; // FIX: unlatch so a later explicit re-trigger (e.g. after cloud sync) can run
            window._bzAutoPlayAfterRestore = false;
            if (typeof updatePlayPauseIcon === 'function') updatePlayPauseIcon();
            window.isShuffling = localStorage.getItem('beatZen_shuffle') === 'true';
            window.repeatMode = parseInt(localStorage.getItem('beatZen_repeat_mode') || '0', 10);
    ...
    window._bzResetRestoredState = function () {
        ...
        // FIX: previously left `restoring` latched true forever after the first
        // restore attempt, which silently blocked every later re-trigger this
        // function is meant to enable (e.g. once cloud-synced song/position
        // data lands after sign-in). Clearing it here makes the reset actually
        // let the mobile engine run fresh, as the comment above always intended.
...
// Gate Sign Up / Sign In / Forgot Password buttons are now wired in auth.js
// (they open the gate's own inline forms instead of routing to Settings).
...
function renderBeatZenUniverseSection(container) {
    ...
    if (cards.length) {
        universeRow = makeRow(cards);
        universeRow.classList.add('bzp-universe-row');
        sec.appendChild(universeRow);
    } else {
    ...
    container.appendChild(sec);
    if (universeRow) attachCustomScrollbar(sec, universeRow);
}
...
function renderPlaylistsNew(container) {
    ...
    if (recommended.length) {
        ...
        attachCustomScrollbar(recSection, recRow);
    }
    ...
    if (listenAgain.length) {
        ...
        laRow.id = 'bzp-la-row';
        laRow.classList.add('bzp-la-row-scroll');
        ...
        attachCustomScrollbar(laSection, laRow);
        ...
        try {
            ...
            if (newSongs.length > 0) {
                ...
                if (window.applySavedTime && audioPlayer && !audioPlayer._restoreApplied) {
                    window._bzRestorePromise = window.applySavedTime();
                }
            }
        } catch (e) {
...
// NOTE: the old "cinematic-card.js" module was removed here — the 9:16
// single-song card is now drawn by the Premium Song Card module at the
// end of this file, which provides window.BZCinematicCard.drawForAlbum().
...
(function () {
    ...
    const RATIOS = [
        // Was 4320x7680 / 7680x4320 (true 8K). That's 4x oversampling over the
        // 1080x1920 design space the cards are drawn in, so every shadowBlur
        // (there are ~10+ per card, some with large radii) was running at 4x
        // the pixel count *and* 4x the blur radius — by far the slowest part
        // of the render. Dropping to 2x oversampling (still a crisp 2160x3840
        // / 3840x2160 export, well above what any share sheet needs) cuts the
        // render to a fraction of the time with no visible quality loss.
        { id: '916', label: '9:16', w: 2160, h: 3840 },
        { id: '169', label: '16:9', w: 3840, h: 2160 },
    ];
    ...
    /* ─── Song pill row ─── */
    // nameSize (optional, px) — a pre-fitted font size for the song name, so every
    // row on a card shares one size (see fitPillNameSize). Falls back to the default.
    function drawPill(ctx, x, y, w, h, theme, num, name, dur, fs, nameSize) {
        ...
        const mid = y + h / 2;
        const nSize = nameSize || Math.round(13 * fs);
        const k = pillSubScale(nSize, Math.round(13 * fs));
        ctx.save();
        ctx.font = `bold ${Math.round(12 * fs * k)}px ${sans}`; ctx.fillStyle = theme.num;
        ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
        ctx.fillText(String(num), x + Math.round(13 * fs), mid); ctx.restore();
        ctx.save();
        ctx.font = `400 ${Math.round(11 * fs * k)}px ${sans}`; ctx.fillStyle = 'rgba(255,255,255,0.38)';
        ...
        ctx.save();
        ctx.font = `500 ${nSize}px ${sans}`; ctx.fillStyle = 'rgba(255,255,255,0.90)';
        ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
        ctx.beginPath(); ctx.rect(nx, y, nmw, h); ctx.clip();
        ctx.fillText(fitText(ctx, name, nmw), nx, mid); ctx.restore();
    }

    // The track number and duration ease down at half the rate the name is
    // shrunk, so a fitted-down name never sits beside oversized digits.
    function pillSubScale(nameSize, base) {
        return 0.5 + 0.5 * Math.min(1, nameSize / base);
    }

    // Largest song-name font size (px) at which EVERY name in `rows` fits its pill
    // in full. Never bigger than the default (13 * fs) and never smaller than
    // `minRatio` of it — below that the "…" fallback in fitText still applies.
    // One shared size keeps all rows on the card looking consistent.
    function fitPillNameSize(ctx, rows, w, fs, minRatio) {
        const sans = `-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif`;
        const base = Math.round(13 * fs);
        const floor = Math.max(1, Math.round(base * minRatio));
        if (!rows || !rows.length) return base;
        ctx.save();
        let size = base;
        for (; size > floor; size--) {
            const k = pillSubScale(size, base);
            const fits = rows.every(r => {
                // same maths as drawPill: number column + duration + right gutter
                ctx.font = `400 ${Math.round(11 * fs * k)}px ${sans}`;
                const avail = w - Math.round(30 * fs) - ctx.measureText(esc(r.duration)).width - Math.round(24 * fs);
                ctx.font = `500 ${size}px ${sans}`;
                return ctx.measureText(esc(r.title)).width <= avail;
            });
            if (fits) break;
        }
        ctx.restore();
    ...
    // Cinematic full-bleed backdrop built from the artwork itself: blurred,
    // colour-graded and vignetted so a floating glass widget reads clearly
    // on top of it — a movie-still mood rather than a flat gradient.
    async function bzDrawCinematicBackdrop(ctx, canvasEl, CW, CH, DPR, album, theme) {
        const img = await loadImage(album.imageUrl || album.albumCover || '');
        ctx.save();
        if (img) {
            // Overscan slightly so the heavy blur never reveals a hard edge
            const scale = Math.max(CW / img.width, CH / img.height) * 1.10;
            const dw = img.width * scale, dh = img.height * scale;
            const dx = (CW - dw) / 2, dy = (CH - dh) / 2;
            ctx.filter = 'blur(30px) saturate(1.15) brightness(0.80)';
            ctx.drawImage(img, dx, dy, dw, dh);
            ctx.filter = 'none';
        } else {
            // No artwork to fall back on — reuse the themed gradient
            bzDrawBackground(ctx, CW, CH, theme);
        }
        ctx.restore();

        // Theme-tinted colour wash — keeps the style picker meaningful
        ctx.save();
        const wash = ctx.createLinearGradient(0, 0, 0, CH);
        wash.addColorStop(0, bzRgba(theme.accent, 0.20));
        wash.addColorStop(0.45, bzRgba(theme.accent, 0.05));
        wash.addColorStop(1, bzRgba(theme.accent, 0.22));
        ctx.fillStyle = wash;
        ctx.fillRect(0, 0, CW, CH);
        ctx.restore();

        // Vignette + top/bottom fades so the widget and footer stay legible
        ctx.save();
        const vin = ctx.createRadialGradient(CW / 2, CH * 0.42, CH * 0.22, CW / 2, CH * 0.42, CH * 0.78);
        vin.addColorStop(0, 'rgba(0,0,0,0)');
        vin.addColorStop(1, 'rgba(0,0,0,0.55)');
        ctx.fillStyle = vin;
        ctx.fillRect(0, 0, CW, CH);

        const topFade = ctx.createLinearGradient(0, 0, 0, CH * 0.20);
        topFade.addColorStop(0, 'rgba(0,0,0,0.40)');
        topFade.addColorStop(1, 'rgba(0,0,0,0)');
        ctx.fillStyle = topFade;
        ctx.fillRect(0, 0, CW, CH * 0.20);

        const bottomFade = ctx.createLinearGradient(0, CH * 0.60, 0, CH);
    ...
    // Frosted glassmorphism panel: a real backdrop-blur sampled straight off
    // the card itself, a dark translucent slate fill and a hairline glass
    // border — the floating "now playing" widget for the cinematic card.
    function bzDrawGlassWidget(ctx, canvasEl, x, y, w, h, r, DPR) {
        ctx.save();
        roundRect(ctx, x, y, w, h, r);
        ctx.clip();
        try {
            ...
            bctx.drawImage(canvasEl, sx, sy, sw, sh, 0, 0, sw, sh);
            ctx.drawImage(blurCanvas, x, y, w, h);
        } catch (_) { /* blur unsupported — the tinted fill below still reads as glass */ }
        ctx.fillStyle = 'rgba(14,14,24,0.46)';
        ctx.fillRect(x, y, w, h);
        ctx.restore();

        // Soft sheen across the top half, like light catching glass
        ctx.save();
    ...
    // Clear, true-color backdrop: the artwork itself, sharp and unblurred,
    // filling the frame edge-to-edge. Only a light fade at the very bottom
    // for the brand/footer — the photo itself stays fully legible ("clear
    // image"), with colour shown through the frame's own accents instead
    // of a wash over the whole scene.
    async function bzDrawClearBackdrop(ctx, CW, CH, album, theme) {
        const img = await loadImage(album.imageUrl || album.albumCover || '');
        ctx.save();
        if (img) {
        ...
        // Light bottom-only fade — just enough for the brand chip and the
        // promo footer to read, without dulling the photo itself
        ctx.save();
        const bottomFade = ctx.createLinearGradient(0, CH * 0.78, 0, CH);
        bottomFade.addColorStop(0, 'rgba(0,0,0,0)');
        bottomFade.addColorStop(1, 'rgba(0,0,0,0.40)');
        ctx.fillStyle = bottomFade;
        ctx.fillRect(0, CH * 0.78, CW, CH * 0.22);
        ctx.restore();
    }

    // "Frame" widget: a distinct glass treatment from the cinematic style —
    // a soft external drop shadow lifts it off the clear photo beneath it,
    // a theme-coloured top accent strip reads like a "now playing" cue, and
    // a theme-tinted hairline border ties it to the picker. The glass
    // itself still uses a real backdrop-blur sampled from the canvas,
    // local to just this shape — everything outside it stays sharp.
    function bzDrawFrameWidget(ctx, canvasEl, x, y, w, h, r, DPR, theme) {
        // Soft external shadow, cast by an opaque stand-in shape that the
        // glass panel below will fully cover
        ctx.save();
        ctx.shadowColor = 'rgba(0,0,0,0.5)';
        ctx.shadowBlur = Math.round(h * 0.35);
        ctx.shadowOffsetY = Math.round(h * 0.12);
        roundRect(ctx, x, y, w, h, r);
        ctx.fillStyle = 'rgba(0,0,0,0.9)';
        ctx.fill();
        ctx.restore();

        // Frosted glass panel — real, local backdrop-blur
        ctx.save();
        roundRect(ctx, x, y, w, h, r);
        ctx.clip();
        try {
            ...
            bctx.drawImage(canvasEl, sx, sy, sw, sh, 0, 0, sw, sh);
            ctx.drawImage(blurCanvas, x, y, w, h);
        } catch (_) { /* tinted fill below still reads as glass */ }
        ctx.fillStyle = 'rgba(16,15,24,0.50)';
        ctx.fillRect(x, y, w, h);
        ctx.restore();

        // Theme-coloured top accent strip
        ctx.save();
        const stripInset = Math.max(1, Math.round(w / 90));
        const stripH = Math.max(2, Math.round(h * 0.05));
        roundRect(ctx, x + stripInset, y + stripInset, w - stripInset * 2, stripH, stripH / 2);
        ctx.fillStyle = bzRgba(theme.accent, 0.9);
        ctx.fill();
        ctx.restore();

        // Hairline border, tinted with the theme accent
        ctx.save();
        roundRect(ctx, x + 0.5, y + 0.5, w - 1, h - 1, r);
        ctx.strokeStyle = bzRgba(theme.accent, 0.35);
        ctx.lineWidth = Math.max(1, w / 480);
        ctx.stroke();
        ctx.restore();
    }

    // A small glass "chip" behind the BeatZen brand mark so it stays
    // legible against any photo — a quieter echo of the main frame's glass
    // language, reused as-is from the cinematic style.
    function bzDrawBrandChip(ctx, canvasEl, cx, y, theme, fs, DPR) {
        const brandW = brandBlockWidth(ctx, fs);
        const iconR = Math.round(9 * fs);
        const padX = Math.round(12 * fs);
        const padY = Math.round(8 * fs);
        const w = brandW + padX * 2;
        const h = iconR * 2 + padY * 2;
        const x = cx - w / 2;
        const yTop = y - h / 2;
        const r = h / 2;
        bzDrawGlassWidget(ctx, canvasEl, x, yTop, w, h, r, DPR);
        drawBrand(ctx, x + padX, y, theme, fs, 'left');
    }

    // A soft, theme-tinted radial glow behind the vinyl badge — it reads as
    // glowing off the photo, and gets naturally softened wherever it falls
    // under the glass sheet, tying the two pieces together.
    function bzDrawGlow(ctx, cx, cy, r, theme) {
        ctx.save();
        const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, r);
        g.addColorStop(0, bzRgba(theme.accent, 0.55));
    ...
    // Rounded-rect path, top corners only — the bottom sheet's silhouette
    ...
    // Bottom sheet: a full-width glass dock, rounded only at the top
    // corners, rising from the bottom edge — a different silhouette from
    // the floating inset frame used in the earlier styles.
    function bzDrawBottomSheet(ctx, canvasEl, x, y, w, h, r, DPR, theme) {
        // Soft shadow lifting the sheet off the photo above it
        ctx.save();
        ctx.shadowColor = 'rgba(0,0,0,0.55)';
        ctx.shadowBlur = Math.round(h * 0.30);
        ...
        // Frosted glass — real, local backdrop-blur
        ctx.save();
        roundRectTop(ctx, x, y, w, h, r);
        ctx.clip();
        try {
        ...
        fillG.addColorStop(0, 'rgba(14,13,20,0.40)');
        fillG.addColorStop(1, 'rgba(8,7,12,0.72)');
        ctx.fillStyle = fillG;
        ctx.fillRect(x, y, w, h);
        ctx.restore();

        // Theme-coloured hairline across the top edge only
        ctx.save();
        roundRectTop(ctx, x, y, w, h, r);
        ctx.clip();
        ctx.fillStyle = bzRgba(theme.accent, 0.85);
        ctx.fillRect(x, y, w, Math.max(2, h * 0.012));
        ctx.restore();
    }

    // Vinyl badge: the album art as a record label at the centre of a dark
    // grooved disc, with a theme-coloured trim ring — different
    // iconography from the rounded-square thumbnails used before.
    async function bzDrawVinylBadge(ctx, cx, cy, d, album, theme) {
        const r = d / 2;

        // Drop shadow
        ctx.save();
        ctx.shadowColor = 'rgba(0,0,0,0.6)';
        ctx.shadowBlur = Math.round(d * 0.22);
        ctx.shadowOffsetY = Math.round(d * 0.06);
        ...
        // Theme-coloured trim ring around the label
        const labelR = r * 0.62;
        ctx.save();
        ctx.beginPath(); ctx.arc(cx, cy, labelR + Math.max(1, d * 0.02), 0, Math.PI * 2);
        ctx.strokeStyle = bzRgba(theme.accent, 0.9);
        ctx.lineWidth = Math.max(1.5, d * 0.03);
        ctx.stroke();
        ctx.restore();

        // Photo label — circular crop of the artwork
        await bzDrawArtwork(ctx, cx - labelR, cy - labelR, labelR * 2, labelR * 2, album, labelR, false);

        // Spindle hole
        ctx.save();
        ctx.beginPath(); ctx.arc(cx, cy, Math.max(1.5, d * 0.035), 0, Math.PI * 2);
        ctx.fillStyle = '#0a0910'; ctx.fill();
        ctx.restore();
    }
    ...
    async function drawCard(album, theme, ratio) {
        const DPR = 2;
        // 8K export — probe the requested size first and halve it if the
        // device can't actually allocate a canvas that large (otherwise the
        // export would silently come out blank on weaker phones).
        let RW = ratio.w, RH = ratio.h;
        try {
            const probe = document.createElement('canvas');
            probe.width = RW; probe.height = RH;
            const pctx = probe.getContext('2d');
            pctx.fillStyle = '#fff';
            pctx.fillRect(0, 0, 2, 2);
            const probeFailed = probe.width !== RW || probe.height !== RH ||
                pctx.getImageData(0, 0, 1, 1).data[3] === 0;
            // Release the probe's full-size pixel buffer immediately. It used to be
            // left for the GC, so every re-render leaked a canvas-sized buffer and
            // phones (iOS especially) soon started handing back blank canvases.
            probe.width = 0; probe.height = 0;
            if (probeFailed) { RW = Math.round(RW / 2); RH = Math.round(RH / 2); }
        } catch (_) { RW = Math.round(RW / 2); RH = Math.round(RH / 2); }
        const CW = RW / DPR;
        const CH = RH / DPR;
        const canvas = document.createElement('canvas');
        ...
        // SINGLE SONG — "Aurora Spotlight" card, portrait. Centred poster
        // composition: an aurora-graded backdrop pulled from the artwork, a
        // floating glass-edged cover with a gloss sweep, an editorial title
        // block, a live scrubber and a full transport row. Drawn by the
        // Premium Song Card module at the end of this file.
        if (isSingleSong && !isLandscape) {
            await window.BZCinematicCard.drawForAlbum(ctx, canvas, album, theme);
            return canvas;
        }

        // SINGLE SONG — "Widescreen Marquee" card, landscape. Split
        // editorial layout: a crisp cover tile on the left, a serif display
        // title column on the right over a ghosted note watermark, a
        // procedural waveform seeded from the song title, and the card's own
        // bottom edge doubling as the progress bar. Drawn by the Premium Song
        // Card module at the end of this file.
        if (isSingleSong && isLandscape) {
            await window.BZPremiumSongCard.drawWide(ctx, canvas, album, theme);
            return canvas;
        }
        ...
        if (!isLandscape) {
            ...
            // pill height: fixed 3 slots. Every other size on this card is scaled by
            // FS (~3.7x in the 1080-wide design space), so the pill has to be too —
            // a raw 30-46px pill was shorter than its own 13*FS text (no inner space).
            const PILL_H = Math.min(Math.round(40 * FS), Math.max(Math.round(30 * FS),
            ...
            /* max 3 song pills — shared name size, shrunk a little if a title is long */
            const nameSizeP = fitPillNameSize(ctx, preview, CW - PAD * 2, FS, 0.8);
            for (let i = 0; i < nShow; i++) {
                drawPill(ctx, PAD, y, CW - PAD * 2, PILL_H, theme, i + 1, preview[i].title, preview[i].duration, FS, nameSizeP);
                y += PILL_H + PILL_GAP;
            }
        ...
        /* left column — hugs the cover so the song list gets all the spare width.
           (It used to be a fixed 42% of the card with the cover filling ~62% of it,
           which left a big empty gutter and squeezed the names into "…".) */
        ...
        // (also capped by the height left above the promo box, so cover + brand never crowd it)
        const COVER_SZ = Math.round(Math.max(CH * 0.25,
            Math.min(CW * 0.27, CH * 0.54, availCH - BRAND_GAP - BRAND_ROW_H)));
        const coverX = PAD;
        const RIGHT_X = coverX + COVER_SZ + Math.round(PAD * 0.8);
        ...
        const PILL_FS = FS * Math.min(1.35, Math.max(0.45, PILL_H / Math.round(CH * 0.078)));
        const allPillsH = nShow * PILL_H + Math.max(0, nShow - 1) * PILL_GAP;
        ...
        /* max 3 song pills — one shared name size, shrunk as far as needed
           (down to 55% of default) so the full song name is visible */
        const nameSizeL = fitPillNameSize(ctx, preview, RIGHT_W, PILL_FS, 0.55);
        for (let i = 0; i < nShow; i++) {
            drawPill(ctx, RIGHT_X, ry, RIGHT_W, PILL_H, theme, i + 1, preview[i].title, preview[i].duration, PILL_FS, nameSizeL);
            ry += PILL_H + PILL_GAP;
        }
    ...
    // BLOB CACHE — JPEG at high quality instead of lossless PNG. This card is
    // photographic (blurred cover art, gradients, film grain), which is exactly
    // the content PNG compresses worst on; a 0.92-quality JPEG looks visually
    // identical here but comes out several times smaller. PNG is kept only for
    // the clipboard-copy path, since the Clipboard API doesn't reliably accept
    // other image types across browsers.
    function getBlob(format, quality) {
        format = format || 'image/jpeg';
        quality = quality == null ? 0.92 : quality;
        const key = format + ':' + quality;
        const canvas = offscreenCanvas;
        if (!canvas) return Promise.resolve(null);
        if (lastBlob && lastBlob._key === key && lastBlob._canvas === canvas) return Promise.resolve(lastBlob);
        return new Promise(r => canvas.toBlob(b => {
            // The card was re-rendered while this was still encoding: the blob is
            // of a stale (now released) canvas, so never hand it out or cache it.
            if (canvas !== offscreenCanvas || !b || !b.size) { r(null); return; }
            b._key = key; b._canvas = canvas;
            lastBlob = b;
            r(b);
        }, format, quality));
    }

    // Free a canvas's pixel buffer right away instead of waiting for the GC.
    function releaseCanvas(c) {
        try { if (c) { c.width = 0; c.height = 0; } } catch (_) { }
    }

    // Every card paints a fully opaque background, so a canvas that reads back
    // transparent means the browser silently failed to allocate/draw it (this is
    // what "an image with no content" is).
    function isCanvasBlank(canvas) {
        try {
            ...
            return opaque < 24 * 24 * 0.5;
        } catch (_) { return false; }
    }

    // SHARE FILE — the JPEG is encoded in the background as soon as the card has
    // been drawn, so "More" can call navigator.share() straight from the tap.
    // Browsers only honour share() while the tap's user-activation is still live;
    // awaiting a multi-megapixel encode first could burn it, and the share then
    // fell through to a link with no image.
    let readyFile = null;
    let readyFilePromise = null;

    function getReadyFile() {
        return (readyFile && offscreenCanvas && readyFile._canvas === offscreenCanvas) ? readyFile : null;
    }

    // SHARE-SIZED IMAGE — the full 3840x2160 / 2160x3840 render is many MB (the
    // film grain makes it even worse), which is what made sending slow or fail and
    // burned mobile data. Every export (More / Save / Copy) is scaled down first:
    // JPEG capped at 1920px on the long side and ~500 KB (quality steps down until
    // it fits), PNG (clipboard only) capped at 1600px.
    function getShareBlob(canvas, type) {
        type = type || 'image/jpeg';
        const isJpeg = type === 'image/jpeg';
        const MAX_SIDE = isJpeg ? 1920 : 1600;
        ...
        const encode = q => new Promise(r => c.toBlob(b => r(b), type, q));
        return (async () => {
            let blob = null;
            for (const q of (isJpeg ? [0.85, 0.75, 0.65, 0.55] : [1])) {
                blob = await encode(q);
    ...
    function prepareShareFile() {
        const canvas = offscreenCanvas;
        readyFile = null;
        readyFilePromise = getShareBlob(canvas).then(blob => {
            if (!blob || canvas !== offscreenCanvas) return null;
            const f = new File([blob], `beatzen_${safeFile()}.jpg`, { type: 'image/jpeg' });
            f._canvas = canvas;
            readyFile = f;
            return f;
        }).catch(() => null);
        return readyFilePromise;
    }

    // Copy / Save / More stay disabled while a card is being (re)drawn — tapping
    // them mid-render used to find no canvas and share just a link.
    function setActionsEnabled(on) {
        ['bz-btn-copy', 'bz-btn-save', 'bz-btn-more'].forEach(id => {
            [id, id + '-m'].forEach(bid => {
    ...
    async function renderPreview(album, theme, ratio, epoch) {
        ...
        if (!stage) return;
        // A new card is being built, so the old one (and any encode of it) is stale.
        // Free its canvas now — big canvases pile up quickly on phones, and once the
        // browser hits its canvas-memory cap it starts returning blank ones.
        stage.querySelectorAll('canvas').forEach(releaseCanvas);
        offscreenCanvas = null; lastBlob = null; readyFile = null; readyFilePromise = null;
        setActionsEnabled(false);
        stage.innerHTML =
            '<div class="bz-generating">'
            + '<div class="bz-spinner"><i></i><i></i></div>'
            + '<span class="bz-generating-label">Crafting your card'
            + '<b class="bz-generating-dots"><i>.</i><i>.</i><i>.</i></b>'
            + '</span>'
            + '<small class="bz-generating-sub">Mixing colours from the artwork</small>'
            + '</div>';
        try {
            let canvas = await drawCard(album, theme, ratio);
            // A newer render was triggered
            if (epoch !== renderEpoch) { releaseCanvas(canvas); return; }
            // Browser silently failed to draw it (out of canvas memory)? Retry once
            // at half resolution rather than showing / sharing an empty image.
            if (isCanvasBlank(canvas)) {
                releaseCanvas(canvas);
                canvas = await drawCard(album, theme, Object.assign({}, ratio, { w: Math.round(ratio.w / 2), h: Math.round(ratio.h / 2) }));
                if (epoch !== renderEpoch) { releaseCanvas(canvas); return; }
                if (isCanvasBlank(canvas)) throw new Error('card canvas came back blank');
            }
            offscreenCanvas = canvas;
            lastBlob = null;
            // Show the drawn canvas directly instead of round-tripping through
            // toDataURL() first — encoding a full-resolution PNG just to display
            // it adds a visible extra delay on top of the render itself. The
            // canvas is only converted to a PNG/blob later, lazily, when the
            // user actually copies/saves/shares it (see getBlob()).
            canvas.className = 'bz-card-preview';
            canvas.style.cssText = 'display:block;border-radius:12px;box-shadow:0 12px 48px rgba(0,0,0,0.80);max-width:100%;max-height:100%;width:auto;height:auto;object-fit:contain;';
            stage.innerHTML = '';
    ...
    async function handleSave() {
        ...
        const blob = getReadyFile() || await (readyFilePromise || prepareShareFile());
        if (blob) {
            const url = URL.createObjectURL(blob);
            const a = Object.assign(document.createElement('a'), { href: url, download: `beatzen_${safeFile()}.jpg` });
            document.body.appendChild(a); a.click(); a.remove();
            // Revoking straight away cancels the download in some mobile browsers
            setTimeout(() => URL.revokeObjectURL(url), 30000);
            showToast('✓ Image saved!');
        } else { showToast('Card is still being created — try again in a moment.'); }
        setBtnState('bz-btn-save', false);
    }

    async function handleMore() {
        if (!offscreenCanvas) { showToast('Card is still being created — try again in a moment.'); return; }
        setBtnState('bz-btn-more', true, '…');
        try {
            const title = `BeatZen — ${currentAlbum?.title || ''}`;
            // Fast path: the JPEG was pre-encoded, so nothing is awaited before
            // navigator.share() and the tap's user-activation is still valid.
            let file = getReadyFile();
            let waited = false;
            if (!file) { waited = true; file = await (readyFilePromise || prepareShareFile()); }
            if (!file) { showToast('Could not prepare the image — try again.'); return; }

            if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
                try {
                    // Image only. Adding url/text next to the file makes several apps
                    // (Messenger, Telegram, X…) keep the link and silently drop the image.
                    await navigator.share({ files: [file], title });
                } catch (e) {
                    if (e && e.name === 'AbortError') return;          // user closed the sheet
                    if (e && e.name === 'NotAllowedError' && waited) {  // tap expired while encoding
                        showToast('Image is ready — tap More again to share.');
                        return;
                    }
                    console.error('BeatZen share failed', e);
                    await handleSave();
                }
                return;
            }
            // This browser can't share files — put the image on the clipboard, else save it
            if (window.ClipboardItem && navigator.clipboard?.write) {
                try {
                    await navigator.clipboard.write([new ClipboardItem({ 'image/png': getShareBlob(offscreenCanvas, 'image/png') })]);
    ...
    async function handleCopyImage() {
        ...
        if (!offscreenCanvas) { showToast('Nothing to copy yet.'); setBtnState('bz-btn-copy', false); return; }
        if (window.ClipboardItem && navigator.clipboard?.write) {
            try {
                // Pass the blob as a *promise* straight into ClipboardItem and call
                // write() immediately, instead of `await getBlob()` first. Awaiting
                // the PNG encode before calling write() puts an async gap between
                // the click and the clipboard call, which burns the browser's
                // short "user activation" window — some browsers then stall or
                // silently reject it, which is what was making copy feel slow
                // (it was falling through to the much slower Save-to-file path).
                // Calling write() right away keeps it inside that window; the
                // encode still happens in the background either way.
                await navigator.clipboard.write([new ClipboardItem({ 'image/png': getShareBlob(offscreenCanvas, 'image/png') })]);
                showToast('✓ Image copied!');
            }
...
(function () {
    ...
    function stampSectionIds() {
        var pc = document.getElementById('playlists-container');
        if (!pc) return;
        // Scoped to .bzp-section so that, when the user has no playlists
        // and that section isn't rendered, a year section further down
        // can't be stamped with the Your Playlists id by mistake.
        var yourSec = pc.querySelector('.bzp-section.dp-section');
    ...
    var PILL_TARGETS = ['bzp-your-playlists', 'bzp-mfy-section', 'bzp-universe-wrap', 'bzp-rec-section', 'bzp-artists-section', 'bzp-heroes-section', 'bzp-la-section'];
...
(function () {
    ...
    const NAV_ORDER = ['home-link', 'search-link', 'playlists-link', 'settings-link'];
...
/* ============================================================================
   BEAT ZEN — Premium Song Share Cards  (v2)
   ----------------------------------------------------------------------------
   Two brand-new, visually unrelated card designs:

     • PORTRAIT  9:16  — "AURORA SPOTLIGHT"
         Centred poster composition. Blurred + aurora-graded backdrop pulled
         from the artwork itself, a floating glass-edged cover with a gloss
         sweep, an editorial title block, a real scrubber and a full transport
         row with a glowing play button.

     • LANDSCAPE 16:9  — "WIDESCREEN MARQUEE"
         Split editorial layout. Crisp cover tile on the left, a serif display
         title column on the right over a ghosted note watermark, a procedural
         waveform seeded from the song title, and the card's own bottom edge
         doubling as the progress bar.

   Both auto-extract an accent palette from the cover art, so every song gets
   a different-looking card while staying on brand.

   INTEGRATION — see the notes at the bottom of this file.
   ==========================================================================*/
...
(function () {
    ...
    /* ---------------------------------------------------------------- sizes */
    const PW = 1080, PH = 1920;   // portrait  design space
    const LW = 1920, LH = 1080;   // landscape design space
    ...
    function ellipsize(ctx, text, maxW) {
        let t = esc(text);
        if (!t || ctx.measureText(t).width <= maxW) return t;
        while (t.length > 1 && ctx.measureText(t + '…').width > maxW) t = t.slice(0, -1);
        return t.replace(/[\s,–-]+$/, '') + '…';
    }

    /* Greedy word-wrap capped at maxLines, ellipsizing the final line if the
       text still overflows. ctx.font must already be set to the size the
       caller intends to draw with. Used so long artist names get a second
       line instead of being cut off after the first. */
    function wrapLines(ctx, text, maxW, maxLines) {
        ...
        if (lines.length > maxLines) {
            const extra = lines.slice(maxLines - 1).join(' ');
            lines.length = maxLines - 1;
            lines.push(ellipsize(ctx, extra, maxW));
        }
        return lines;
    }

    /* Fits a title into <= maxLines, shrinking the size until it balances. */
    function fitTitle(ctx, text, opts) {
        const words = esc(text).split(/\s+/).filter(Boolean);
        const setFont = opts.font;
        if (!words.length) return { fs: opts.minFS, lines: [''] };

        for (let fs = opts.startFS; fs >= opts.minFS; fs -= 2) {
            ctx.font = setFont(fs);
            const lines = [];
            let line = '';
            let ok = true;
            for (let i = 0; i < words.length; i++) {
                const test = line ? line + ' ' + words[i] : words[i];
                if (ctx.measureText(test).width <= opts.maxW) { line = test; continue; }
                if (!line) { ok = false; break; }              // single word too wide
                lines.push(line); line = words[i];
                if (lines.length === opts.maxLines) { ok = false; break; }
            }
            if (!ok) continue;
            if (line) lines.push(line);
            if (lines.length <= opts.maxLines) return { fs: fs, lines: lines };
        }

        ctx.font = setFont(opts.minFS);
        const lines = [];
        let line = '';
        for (let i = 0; i < words.length; i++) {
            const test = line ? line + ' ' + words[i] : words[i];
            if (ctx.measureText(test).width <= opts.maxW || !line) line = test;
            else { lines.push(line); line = words[i]; }
            if (lines.length === opts.maxLines) break;
        }
        if (lines.length < opts.maxLines && line) lines.push(line);
        if (lines.length === opts.maxLines) {
            lines[opts.maxLines - 1] = ellipsize(ctx, lines[opts.maxLines - 1], opts.maxW);
        }
        return { fs: opts.minFS, lines: lines };
    }
    ...
    function ensureFonts() {
        ...
        _fonts = (async function () {
            try {
                // Poppins is already linked (and preloaded) in index.html, so
                // only Playfair Display needs fetching here — requesting both
                // every time just adds a redundant wait on the one already cached.
                if (!document.getElementById('bz-card-fonts')) {
                    const l = document.createElement('link');
                    l.id = 'bz-card-fonts';
                    l.rel = 'stylesheet';
                    l.href = 'https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,600;0,700;1,500&display=swap';
                    document.head.appendChild(l);
                }
                if (!document.fonts || !document.fonts.load) return;
                // Only wait on the exact weights this card draws with — NOT
                // document.fonts.ready, which blocks until every font on the
                // whole page settles and can stall the first render for a
                // while longer than the card actually needs.
                await Promise.all([
                    document.fonts.load('500 40px Poppins'),
                    document.fonts.load('600 40px Poppins'),
                    document.fonts.load('700 40px Poppins'),
                    document.fonts.load('700 40px Poppins', '\u20B9'),   // rupee sign lives in Poppins' devanagari subset
            ...
            } catch (_) { /* system stack is a fine fallback */ }
        })();
        return _fonts;
    }
    ...
    /* Pulls a usable accent out of the cover art. Falls back to the share
       modal's current theme accent when the image is missing or CORS-tainted. */
    function extractPalette(img, fallbackAccent) {
        const out = {
            accent: fallbackAccent || FALLBACK.accent,
            accentAlt: FALLBACK.accentAlt,
            deep: FALLBACK.deep,
            mid: FALLBACK.mid
        };
        if (!img) { deriveFrom(out.accent); return out; }
        ...
        try {
            ...
            const sat = new Array(18).fill(0);
            const lum = new Array(18).fill(0);
            let any = 0;

            for (let i = 0; i < d.length; i += 4) {
                const hsl = rgbToHsl(d[i], d[i + 1], d[i + 2]);
                if (hsl[2] < 0.12 || hsl[2] > 0.93 || hsl[1] < 0.14) continue;
                const b = Math.floor(hsl[0] / 20) % 18;
                const w = hsl[1] * (1 - Math.abs(hsl[2] - 0.55));
                bins[b] += w; sat[b] += hsl[1] * w; lum[b] += hsl[2] * w; any++;
            }
            ...
            if (any > 12) {
                ...
                const h = best * 20 + 10;
                const s = clamp(sat[best] / bins[best], 0.45, 0.92);
                out.accent = hslToHex(h, clamp(s + 0.08, 0.5, 0.9), 0.60);
                out.accentAlt = hslToHex(h + 38, clamp(s, 0.45, 0.85), 0.64);
                out.deep = hslToHex(h - 6, 0.52, 0.045);
                out.mid = hslToHex(h + 14, 0.46, 0.135);
                return out;
            }
        } catch (_) { /* tainted canvas — fall through */ }

        deriveFrom(out.accent);
        return out;

        function deriveFrom(hex) {
            const [r, g, b] = hexToRgb(hex);
            const hsl = rgbToHsl(r, g, b);
            out.accentAlt = hslToHex(hsl[0] + 38, clamp(hsl[1], 0.45, 0.85), 0.64);
            out.deep = hslToHex(hsl[0] - 6, 0.52, 0.045);
            out.mid = hslToHex(hsl[0] + 14, 0.46, 0.135);
        }
    }

    /* Palette from the share modal's selected colour style (Red / Blue /
       Yellow / ...). The card paints its own opaque backdrop over the
       modal's theme background, so without this the style tabs had no
       visible effect — the colours always came from the cover art. */
    function paletteFromTheme(theme) {
        const lum = function (hex) { const c = hexToRgb(hex); return c[0] + c[1] + c[2]; };
        const shades = (theme.bg || []).slice().sort(function (a, b) { return lum(a) - lum(b); });
        return {
            accent: theme.accent,
            accentAlt: theme.accentAlt || theme.accent,
            deep: shades[0] || FALLBACK.deep,
            mid: shades[shades.length - 1] || FALLBACK.mid,
            themed: true
        };
    }

    /* Selected colour style wins; cover-art colours are only the fallback. */
    function paletteForCard(img, theme, fallbackAccent) {
        return (theme && theme.accent) ? paletteFromTheme(theme) : extractPalette(img, fallbackAccent);
    }

    /* Cheap, universally-supported blur: downscale hard, upscale smooth. */
    function drawBlurredCover(ctx, img, x, y, w, h, alpha, px, tint) {
        if (!img) return;
        const n = px || 34;
        const small = document.createElement('canvas');
        small.width = n; small.height = Math.max(1, Math.round(n * (h / w)));
        const sg = small.getContext('2d');
        const s = Math.max(small.width / img.width, small.height / img.height);
        const dw = img.width * s, dh = img.height * s;
        sg.drawImage(img, (small.width - dw) / 2, (small.height - dh) / 2, dw, dh);
        if (tint) {
            /* recolour the blur to the chosen style's hue, keeping the photo's light/dark texture */
            sg.globalCompositeOperation = 'color';
            sg.fillStyle = tint;
    ...
    /* Cover-fit artwork into a rounded rect. */
    function drawCoverFit(ctx, img, x, y, w, h, r) {
        ctx.save();
        roundRect(ctx, x, y, w, h, r);
        ctx.clip();
        if (img) {
    ...
    function drawAurora(ctx, w, h, pal, spots) {
        ctx.save();
        ctx.globalCompositeOperation = 'screen';
        spots.forEach(function (s) {
            const g = ctx.createRadialGradient(s.x * w, s.y * h, 0, s.x * w, s.y * h, s.r * w);
            g.addColorStop(0, rgba(s.alt ? pal.accentAlt : pal.accent, s.a));
            g.addColorStop(0.55, rgba(s.alt ? pal.accentAlt : pal.accent, s.a * 0.28));
            g.addColorStop(1, rgba(s.alt ? pal.accentAlt : pal.accent, 0));
            ctx.fillStyle = g;
            ctx.beginPath();
    ...
    function iconNote(ctx, cx, cy, s, color) {
        ctx.save();
        ctx.fillStyle = color; ctx.strokeStyle = color;
        ctx.lineWidth = s * 0.11; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
        const stemX = cx + s * 0.20, top = cy - s * 0.46, bot = cy + s * 0.26;
        ctx.beginPath(); ctx.moveTo(stemX, bot); ctx.lineTo(stemX, top); ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(stemX, top);
        ctx.quadraticCurveTo(cx + s * 0.62, top + s * 0.05, cx + s * 0.50, top + s * 0.30);
        ctx.stroke();
        ctx.beginPath();
        ctx.ellipse(cx - s * 0.05, bot, s * 0.26, s * 0.20, -0.32, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
    }

    function iconEq(ctx, x, cy, unit, color, seedBars) {
        const bars = seedBars || [0.55, 1, 0.72, 0.38];
        ctx.save();
        ctx.fillStyle = color;
        bars.forEach(function (v, i) {
            const hgt = unit * 1.9 * v;
            roundRect(ctx, x + i * unit * 1.55, cy - hgt / 2, unit * 0.72, hgt, unit * 0.36);
            ctx.fill();
        });
        ctx.restore();
    }

    function iconHeart(ctx, cx, cy, s, color, filled) {
        ctx.save();
        ctx.beginPath();
        ctx.moveTo(cx, cy + s * 0.38);
        ctx.bezierCurveTo(cx - s * 0.95, cy - s * 0.20, cx - s * 0.42, cy - s * 0.78, cx, cy - s * 0.26);
        ctx.bezierCurveTo(cx + s * 0.42, cy - s * 0.78, cx + s * 0.95, cy - s * 0.20, cx, cy + s * 0.38);
        ctx.closePath();
        if (filled) { ctx.fillStyle = color; ctx.fill(); }
        else { ctx.strokeStyle = color; ctx.lineWidth = s * 0.16; ctx.stroke(); }
        ctx.restore();
    }

    function iconSkip(ctx, cx, cy, s, color, dir) {
        ctx.save();
        ctx.translate(cx, cy);
        ctx.scale(dir, 1);
        ctx.fillStyle = color;
        ctx.beginPath();
        ctx.moveTo(-s * 0.55, -s * 0.5); ctx.lineTo(s * 0.10, 0); ctx.lineTo(-s * 0.55, s * 0.5);
        ctx.closePath(); ctx.fill();
        roundRect(ctx, s * 0.22, -s * 0.5, s * 0.20, s, s * 0.09);
        ctx.fill();
        ctx.restore();
    }
    ...
    function iconShuffle(ctx, cx, cy, s, color) {
        ...
        ctx.beginPath();
        ctx.moveTo(cx - s * 0.62, cy - s * 0.40);
        ctx.bezierCurveTo(cx - s * 0.10, cy - s * 0.40, cx + s * 0.05, cy + s * 0.40, cx + s * 0.50, cy + s * 0.40);
        ctx.moveTo(cx - s * 0.62, cy + s * 0.40);
        ctx.bezierCurveTo(cx - s * 0.10, cy + s * 0.40, cx + s * 0.05, cy - s * 0.40, cx + s * 0.50, cy - s * 0.40);
        ctx.stroke();
        [-1, 1].forEach(function (d) {
            ctx.beginPath();
            ctx.moveTo(cx + s * 0.38, cy + d * s * 0.40 - s * 0.20);
            ctx.lineTo(cx + s * 0.64, cy + d * s * 0.40);
            ctx.lineTo(cx + s * 0.38, cy + d * s * 0.40 + s * 0.20);
            ctx.closePath(); ctx.fill();
        });
        ctx.restore();
    }
    ...
    function iconRepeat(ctx, cx, cy, s, color) {
        ...
        ctx.lineWidth = s * 0.14; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
        const r = s * 0.46;
        ctx.beginPath(); ctx.arc(cx, cy, r, -Math.PI * 0.86, Math.PI * 0.62); ctx.stroke();
        ctx.beginPath(); ctx.arc(cx, cy, r, Math.PI * 0.86, Math.PI * 1.62); ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(cx + r * 0.52, cy - r * 1.06);
        ctx.lineTo(cx + r * 0.16, cy - r * 0.74);
        ctx.lineTo(cx + r * 0.62, cy - r * 0.44);
        ctx.closePath(); ctx.fill();
        ctx.restore();
    }

    function iconPlayPause(ctx, cx, cy, s, color, paused) {
        ctx.save();
        ctx.fillStyle = color;
        if (paused) {
            ctx.beginPath();
            ctx.moveTo(cx - s * 0.30, cy - s * 0.46);
            ctx.lineTo(cx + s * 0.44, cy);
            ctx.lineTo(cx - s * 0.30, cy + s * 0.46);
            ctx.closePath(); ctx.fill();
        } else {
            roundRect(ctx, cx - s * 0.36, cy - s * 0.46, s * 0.24, s * 0.92, s * 0.10); ctx.fill();
            roundRect(ctx, cx + s * 0.12, cy - s * 0.46, s * 0.24, s * 0.92, s * 0.10); ctx.fill();
        }
        ctx.restore();
    }
    ...
    function parseTime(t) {
        ...
        if (p.length === 3 && p.every(function (n) { return isFinite(n); })) return p[0] * 3600 + p[1] * 60 + p[2];
        return 0;
    }
    ...
    function readPlayback(album) {
        let total = parseTime((album.songs && album.songs[0] && album.songs[0].duration) || album.duration || '3:30') || 210;
        let current = total * 0.32;
        let paused = false;
        try {
            const playing = (window.playingAlbum && window.playingAlbum.songs)
                ? window.playingAlbum.songs[window.currentSongIndex] : null;
            const el = window.audioPlayer || document.getElementById('audio-player');
            if (playing && String(playing.id) === String(album.id) && el && isFinite(el.currentTime)) {
        ...
        return {
            current: current, total: total, paused: paused,
            currentText: fmtTime(current), totalText: fmtTime(total),
            progress: clamp(total ? current / total : 0.32, 0.02, 0.99)
        };
    }

    function isFavourite(id) {
        try {
            const list = JSON.parse(localStorage.getItem('beatZen_favourites') || '[]');
            return list.some(function (s) { return String(s && s.id != null ? s.id : s) === String(id); });
        } catch (_) { return false; }
    }

    function readState(album, theme) {
        const p = readPlayback(album);
        return {
            title: esc(album.title || album.name || 'Untitled'),
            movie: esc(album._albumName || ''),
            artist: esc(album._artistName || ''),
            liked: isFavourite(album.id),
            themeAccent: (theme && theme.accent) || FALLBACK.accent,
            play: p
        };
    }

    function setLetterSpacing(ctx, px) {
        try { ctx.letterSpacing = px ? px + 'px' : '0px'; } catch (_) { }
    }

    /* ==========================================================================
       GLASS PROMO FRAME — frosted panel with a neon border, tinted from the
       album's own accent colours, carrying the single promo line.
       Returns its height; pass draw=false to only measure it.
       ======================================================================= */

    const PROMO_LINE = 'Listen to the full album on BEATZEN for just \u20B99/- month';
    /* Portrait (9:16) card: wider panel, tighter side padding, pure-white text with a
       soft shadow and a darker panel base so the single line stays readable on phones. */
    const PROMO_OPTS_PORTRAIT = { padXk: 0.5, crisp: true, baseAlpha: 0.78 };

    function drawGlassPromo(ctx, x, y, w, pal, fs, align, draw, opts) {
        opts = opts || {};
        const padX = Math.round(fs * (opts.padXk || 1.0)), padY = Math.round(fs * 0.95);
        const innerW = w - padX * 2;

        /* One single line, scaled so the text spans the full inner width of the
           panel (same behaviour for 16:9 and 9:16). `fs` is only the starting size. */
        ctx.save();
        setLetterSpacing(ctx, 0.3);
        ctx.font = '700 ' + fs + 'px ' + SANS;
        let textFS = fs * innerW / Math.max(1, ctx.measureText(PROMO_LINE).width);
        textFS = Math.min(textFS, fs * 1.35);
        ctx.font = '700 ' + textFS + 'px ' + SANS;
        const measured = ctx.measureText(PROMO_LINE).width;
        if (measured > innerW) textFS *= innerW / measured;   // letter-spacing safety
        textFS = Math.floor(textFS * 10) / 10;
        setLetterSpacing(ctx, 0);
        ctx.restore();

        const lineLH = Math.round(textFS * 1.3);
        const h = padY * 2 + lineLH;
        if (draw === false) return h;

        const r = Math.round(fs * 1.1);
        /* dark glass base */
        ctx.save();
        roundRect(ctx, x, y, w, h, r);
        ctx.fillStyle = rgba(pal.deep, opts.baseAlpha || 0.55);
        ctx.fill();
        ctx.restore();

        /* glass body: frosted gradient and diagonal sheen */
        ctx.save();
        roundRect(ctx, x, y, w, h, r);
        ctx.clip();
        const body = ctx.createLinearGradient(x, y, x, y + h);
        body.addColorStop(0, 'rgba(255,255,255,0.14)');
        body.addColorStop(0.5, 'rgba(255,255,255,0.05)');
        body.addColorStop(1, rgba(pal.accent, 0.14));
        ctx.fillStyle = body;
        ctx.fillRect(x, y, w, h);
        ...
        edge.addColorStop(0, pal.accentAlt);
        edge.addColorStop(0.5, pal.accent);
        edge.addColorStop(1, pal.accentAlt);
        roundRect(ctx, x, y, w, h, r);
        ctx.lineWidth = 4;
        ...
        /* text — single line, centred, spanning the panel */
        ctx.save();
        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';
        ctx.font = '700 ' + textFS + 'px ' + SANS;
        ctx.fillStyle = opts.crisp ? '#ffffff' : 'rgba(255,255,255,0.92)';
        if (opts.crisp) {
            ctx.shadowColor = 'rgba(0,0,0,0.55)';
            ctx.shadowBlur = 6;
            ctx.shadowOffsetY = 2;
        ...
        setLetterSpacing(ctx, 0.3);
        ctx.fillText(PROMO_LINE, x + w / 2, y + padY + Math.round((lineLH - textFS) / 2));
        setLetterSpacing(ctx, 0);
        ctx.restore();
        return h;
    }
    ...
    function paintPortrait(ctx, canvasEl, st, img, pal) {
        const W = PW, H = PH;
        const M = 96;
        ...
        bg.addColorStop(0.48, pal.deep);
        bg.addColorStop(1, '#03030A');
        ctx.fillStyle = bg;
        ctx.fillRect(0, 0, W, H);

        drawBlurredCover(ctx, img, -W * 0.18, -H * 0.10, W * 1.36, H * 0.92, 0.50, 30, pal.themed ? pal.accent : null);

        const scrim = ctx.createLinearGradient(0, 0, 0, H);
        scrim.addColorStop(0, rgba(pal.deep, 0.40));
        scrim.addColorStop(0.38, rgba(pal.deep, 0.78));
        scrim.addColorStop(0.72, rgba(pal.deep, 0.97));
        scrim.addColorStop(1, 'rgba(3,3,10,1)');
        ctx.fillStyle = scrim;
        ctx.fillRect(0, 0, W, H);

        drawAurora(ctx, W, H, pal, [
            { x: 1.04, y: 0.12, r: 0.62, a: 0.30 },
            { x: -0.10, y: 0.46, r: 0.50, a: 0.20, alt: true },
            { x: 0.50, y: 1.06, r: 0.72, a: 0.24 }
        ]);

        /* ---- measure the text block first, so the cover (and the logo
           sitting right above it) can size and centre themselves ---- */
        const TEXT_MAXW = W - M * 2;
        const titleFit = fitTitle(ctx, st.title, {
            maxW: TEXT_MAXW, maxLines: 1, startFS: 86, minFS: 28,
            font: function (fs) { return '800 ' + fs + 'px ' + SANS; }
        });
        const titleLH = Math.round(titleFit.fs * 1.16);
        const hasMovie = !!st.movie;
        const hasArtist = !!st.artist;

        /* artist gets up to 2 lines instead of being cut off after one */
        const ARTIST_FS = 38, ARTIST_LH = 56;
        ctx.save();
        ctx.font = '500 ' + ARTIST_FS + 'px ' + SANS;
        setLetterSpacing(ctx, 0.6);
        const artistLines = hasArtist ? wrapLines(ctx, st.artist, TEXT_MAXW, 2) : [];
        setLetterSpacing(ctx, 0);
        ctx.restore();

        const movieH = hasMovie ? 66 : 0;
        const artistH = hasArtist ? artistLines.length * ARTIST_LH : 0;
        const textH = titleFit.lines.length * titleLH + (hasMovie ? movieH + 28 : 0) + (hasArtist ? artistH + 22 : 0);

        /* ---- brand mark: sized here so its row height can be reserved
           above the cover, then drawn once ART_TOP/artX are known ---- */
        const LOGO = 76;
        const WORD_FS = 34;
        const WORD_GAP = 20;
        const LOGO_COVER_GAP = 44;   // breathing room between the logo row and the cover's top edge
        const LOGO_INSET = 36;       // shifts the logo right of the cover's left edge (raise to move it further right)

        const TOP_MIN = 84;
        const TEXT_BOTTOM = H - M;         // no scrubber/transport row anymore — use full height
        const ART_GAP = 72;
        // Cap the cover at the same left/right margin (M) everything else on the
        // card uses. The previous cap (940) could exceed W - 2*M, letting the
        // cover — and the logo pinned to its edge — sit closer to the left
        // border than the title/artist text below, which is what threw the
        // left alignment off. Capping here keeps the logo's left edge locked
        // to the same M-wide gap as the rest of the card.
        const MAX_ART = 780;   // compact cover (was W - M * 2 = 888); must stay <= W - M * 2
        const artSize = clamp(TEXT_BOTTOM - TOP_MIN - LOGO - LOGO_COVER_GAP - ART_GAP - textH, 560, MAX_ART);

        /* centre the whole (logo + cover + gap + text) block vertically in
           the available space, now that the scrubber/transport row is gone */
        const contentH = LOGO + LOGO_COVER_GAP + artSize + ART_GAP + textH;
        const availH = TEXT_BOTTOM - TOP_MIN;
        const blockTop = Math.max(TOP_MIN, (H - contentH) * 0.25);   // 25% of the free space above, 75% below

        const artX = (W - artSize) / 2;
        const ART_TOP = blockTop + LOGO + LOGO_COVER_GAP;
        const logoY = blockTop + LOGO / 2;
        const logoX = artX + LOGO_INSET;

        /* logo chip + BEATZEN wordmark, left-aligned to the cover's own left
           edge and sitting right on top of its border */
        ctx.save();
        ctx.shadowColor = rgba(pal.accent, 0.55);
        ctx.shadowBlur = 40;
        const lg = ctx.createLinearGradient(logoX, logoY - LOGO / 2, logoX + LOGO, logoY + LOGO / 2);
        lg.addColorStop(0, pal.accent);
        lg.addColorStop(1, pal.accentAlt);
        roundRect(ctx, logoX, logoY - LOGO / 2, LOGO, LOGO, 24);
        ctx.fillStyle = lg;
        ctx.fill();
        ctx.restore();
        iconNote(ctx, logoX + LOGO / 2, logoY, LOGO * 0.54, '#FFFFFF');

        ctx.save();
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.font = '800 ' + WORD_FS + 'px ' + SANS;
        ctx.fillStyle = '#FFFFFF';
        ctx.shadowColor = 'rgba(0,0,0,0.4)';
        ctx.shadowBlur = 16;
        setLetterSpacing(ctx, 2);
        ctx.fillText('BEATZEN', logoX + LOGO + WORD_GAP, logoY + 2);
        setLetterSpacing(ctx, 0);
        ctx.restore();

        /* liked marker mirrors the logo on the cover's right edge */
        if (st.liked) iconHeart(ctx, artX + artSize - LOGO_INSET - 20, logoY, 22, pal.accentAlt, true);

        /* ---- cover artwork ---- */
        const R = Math.round(artSize * 0.075);

        ctx.save();
        ctx.shadowColor = rgba(pal.accent, 0.46);
        ctx.shadowBlur = 130;
        ctx.shadowOffsetY = 46;
        roundRect(ctx, artX, ART_TOP, artSize, artSize, R);
        ctx.fillStyle = '#000';
        ctx.fill();
        ctx.restore();

        drawCoverFit(ctx, img, artX, ART_TOP, artSize, artSize, R);

        /* gloss sweep */
        ctx.save();
        roundRect(ctx, artX, ART_TOP, artSize, artSize, R);
        ctx.clip();
        const gl = ctx.createLinearGradient(artX, ART_TOP, artX + artSize * 0.82, ART_TOP + artSize * 0.82);
        gl.addColorStop(0, 'rgba(255,255,255,0.20)');
        gl.addColorStop(0.42, 'rgba(255,255,255,0.05)');
        gl.addColorStop(0.62, 'rgba(255,255,255,0)');
        ctx.fillStyle = gl;
        ctx.fillRect(artX, ART_TOP, artSize, artSize);
        ctx.restore();

        /* hairline + accent ring */
        ctx.save();
        roundRect(ctx, artX + 1, ART_TOP + 1, artSize - 2, artSize - 2, R - 1);
        const hair = ctx.createLinearGradient(artX, ART_TOP, artX + artSize, ART_TOP + artSize);
        hair.addColorStop(0, 'rgba(255,255,255,0.55)');
        hair.addColorStop(0.5, 'rgba(255,255,255,0.10)');
        hair.addColorStop(1, rgba(pal.accent, 0.55));
        ...
        /* ---- title block ---- */
        let ty = ART_TOP + artSize + ART_GAP;
        ctx.save();
        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';
        ctx.font = '800 ' + titleFit.fs + 'px ' + SANS;
        ctx.fillStyle = '#FFFFFF';
        ctx.shadowColor = 'rgba(0,0,0,0.5)';
        ctx.shadowBlur = 24;
        setLetterSpacing(ctx, -1);
        titleFit.lines.forEach(function (ln, i) {
            ctx.fillText(ln, W / 2, ty + i * titleLH);
        });
        setLetterSpacing(ctx, 0);
        ctx.restore();
        ty += titleFit.lines.length * titleLH;
        ...
        if (hasMovie) {
            ...
            ctx.fillText(ellipsize(ctx, 'From “' + st.movie + '”', TEXT_MAXW), W / 2, ty);
            ctx.restore();
            ty += movieH;
        }
        ...
        if (hasArtist) {
            ...
            setLetterSpacing(ctx, 0.6);
            artistLines.forEach(function (ln, i) {
                ctx.fillText(ln, W / 2, ty + i * ARTIST_LH);
            });
            setLetterSpacing(ctx, 0);
            ctx.restore();
        }

        /* glass promo frame — under the artist line, in the empty lower area */
        const promoW = W - 96;
        const promoH = drawGlassPromo(ctx, 0, 0, promoW, pal, 36, 'center', false, PROMO_OPTS_PORTRAIT);
        const promoY = Math.min(ART_TOP + artSize + ART_GAP + textH + 44, H - M - promoH);
        drawGlassPromo(ctx, (W - promoW) / 2, promoY, promoW, pal, 36, 'center', true, PROMO_OPTS_PORTRAIT);
        /* footer rule + text removed */

        drawGrain(ctx, W, H, 0.055);
    ...
    /* seededRandom kept for future use — the waveform it seeded is gone */
    function seededRandom(str) {
        let h = 2166136261;
        const s = String(str || 'beatzen');
    ...
    function paintWide(ctx, canvasEl, st, img, pal) {
        const W = LW, H = LH;
        const M = 104;
        ...
        drawBlurredCover(ctx, img, -W * 0.06, -H * 0.24, W * 1.12, H * 1.48, 0.42, 30, pal.themed ? pal.accent : null);

        const scrim = ctx.createLinearGradient(0, 0, W, 0);
        scrim.addColorStop(0, rgba(pal.deep, 0.92));
        scrim.addColorStop(0.42, rgba(pal.deep, 0.86));
        scrim.addColorStop(1, rgba(pal.deep, 0.96));
        ctx.fillStyle = scrim;
        ctx.fillRect(0, 0, W, H);

        drawAurora(ctx, W, H, pal, [
            { x: 0.18, y: -0.06, r: 0.34, a: 0.26 },
            { x: 1.02, y: 0.92, r: 0.40, a: 0.24, alt: true }
        ]);

        /* ---- layout, measured up-front so the watermark can centre on the song details ---- */
        const ART = 640;                    // compact cover (was 872, then 780)
        const artX = M, artY = Math.round((H - ART) / 2);   // vertically centred
        const R = 44;
        const colX = artX + ART + 96;
        const colW = W - M - colX;

        const LOGO = 62, brandY = artY + 6;   // brand row lines up with the cover's top
        const WORD_FS = 28, WORD_GAP = 18;
        const LOGO_TITLE_GAP = 40;            // gap below the brand row (was 72)

        const titleFit = fitTitle(ctx, st.title, {
            maxW: colW, maxLines: 1, startFS: 104, minFS: 28,
            font: function (fs) { return '800 ' + fs + 'px ' + SANS; }
        });
        const titleLH = Math.round(titleFit.fs * 1.12);

        const ARTIST_FS = 36, ARTIST_LH = 54;
        let artistLines = [];
        if (st.artist) {
            ctx.save();
            ctx.font = '500 ' + ARTIST_FS + 'px ' + SANS;
            setLetterSpacing(ctx, 0.6);
            artistLines = wrapLines(ctx, st.artist, colW, 2);
            setLetterSpacing(ctx, 0);
            ctx.restore();
        }

        const titleTop = brandY + LOGO + LOGO_TITLE_GAP;
        let detailsBottom = titleTop + titleFit.lines.length * titleLH;
        if (st.movie) detailsBottom += 22 + 66;
        if (st.artist) detailsBottom += 30 + artistLines.length * ARTIST_LH;

        /* ---- cover tile, left ---- */

        ctx.save();
        ctx.shadowColor = rgba(pal.accent, 0.42);
        ctx.shadowBlur = 110;
        ctx.shadowOffsetX = 18;
        ctx.shadowOffsetY = 26;
        roundRect(ctx, artX, artY, ART, ART, R);
        ctx.fillStyle = '#000';
        ctx.fill();
        ctx.restore();

        drawCoverFit(ctx, img, artX, artY, ART, ART, R);

        ctx.save();
        roundRect(ctx, artX, artY, ART, ART, R);
        ctx.clip();
        const gl = ctx.createLinearGradient(artX, artY, artX + ART, artY + ART * 0.6);
        ...
        ctx.save();
        roundRect(ctx, artX + 1, artY + 1, ART - 2, ART - 2, R - 1);
        const hair = ctx.createLinearGradient(artX, artY, artX + ART, artY + ART);
        hair.addColorStop(0, 'rgba(255,255,255,0.5)');
        hair.addColorStop(0.55, 'rgba(255,255,255,0.08)');
        hair.addColorStop(1, rgba(pal.accentAlt, 0.5));
        ctx.strokeStyle = hair;
        ctx.lineWidth = 2.2;
        ctx.stroke();
        ctx.restore();
        ...
        /* brand lockup, top of the column: icon chip + BEATZEN wordmark */
        ctx.save();
        ctx.shadowColor = rgba(pal.accent, 0.5);
        ctx.shadowBlur = 30;
        const lg = ctx.createLinearGradient(colX, brandY, colX + LOGO, brandY + LOGO);
        lg.addColorStop(0, pal.accent);
        lg.addColorStop(1, pal.accentAlt);
        roundRect(ctx, colX, brandY, LOGO, LOGO, 20);
        ctx.fillStyle = lg;
        ctx.fill();
        ctx.restore();
        iconNote(ctx, colX + LOGO / 2, brandY + LOGO / 2, LOGO * 0.54, '#FFFFFF');
        ...
        ctx.shadowBlur = 14;
        setLetterSpacing(ctx, 2);
        ctx.fillText('BEATZEN', colX + LOGO + WORD_GAP, brandY + LOGO / 2 + 1);
        setLetterSpacing(ctx, 0);
        ctx.restore();

        if (st.liked) iconHeart(ctx, W - M - 18, brandY + LOGO / 2, 24, pal.accentAlt, true);
        ...
        ctx.shadowBlur = 26;
        setLetterSpacing(ctx, -1);
        titleFit.lines.forEach(function (ln, i) { ctx.fillText(ln, colX, ty + i * titleLH); });
        setLetterSpacing(ctx, 0);
        ctx.restore();
        ty += titleFit.lines.length * titleLH;
        ...
        if (st.movie) {
            ...
            ctx.font = '500 48px ' + SERIF;
            ctx.fillStyle = rgba(pal.accent, 0.92);
            ctx.fillText(ellipsize(ctx, 'From “' + st.movie + '”', colW), colX, ty);
            ctx.restore();
            ty += 66;
        }
        ...
        if (st.artist) {
            ...
            ctx.fillStyle = 'rgba(255,255,255,0.68)';
            artistLines.forEach(function (ln, i) {
                ctx.fillText(ln, colX, ty + i * ARTIST_LH);
            });
            setLetterSpacing(ctx, 0);
            ctx.restore();
            ty += artistLines.length * ARTIST_LH;
        }

        /* glass promo frame — sits just under the text */
        const promoH = drawGlassPromo(ctx, colX, 0, colW, pal, 30, 'left', false);
        const promoY = ty + 44;
        drawGlassPromo(ctx, colX, promoY, colW, pal, 30, 'left');
        /* waveform removed; bottom progress edge removed */
        /* bottom time-code row ("0:00 ... BEATZEN.IN · 4:47") removed */
    ...
    function withDesignSpace(ctx, canvasEl, DWd, fn) {
        const scale = canvasEl.width / DWd;
        ctx.save();
    ...
    async function drawPortrait(ctx, canvasEl, album, theme) {
        await ensureFonts();
        const st = readState(album, theme);
        const img = await loadImage(album.imageUrl || album.albumCover || '');
        const pal = paletteForCard(img, theme, st.themeAccent);
        withDesignSpace(ctx, canvasEl, PW, function () { paintPortrait(ctx, canvasEl, st, img, pal); });
        return canvasEl;
    }

    async function drawWide(ctx, canvasEl, album, theme) {
        await ensureFonts();
        const st = readState(album, theme);
        const img = await loadImage(album.imageUrl || album.albumCover || '');
        const pal = paletteForCard(img, theme, st.themeAccent);
        withDesignSpace(ctx, canvasEl, LW, function () { paintWide(ctx, canvasEl, st, img, pal); });
        return canvasEl;
    }

    window.BZPremiumSongCard = {
        drawPortrait: drawPortrait,
        drawWide: drawWide,
        ensureFonts: ensureFonts,
        PORTRAIT: { w: PW, h: PH },
        WIDE: { w: LW, h: LH },
        _internal: { paintPortrait: paintPortrait, paintWide: paintWide, extractPalette: extractPalette, paletteForCard: paletteForCard, readState: readState }
    };

    /* Drop-in replacement for the old cinematic card: the share module calls
       window.BZCinematicCard.drawForAlbum() for the 9:16 single-song branch,
       so overriding it here swaps the portrait design with no other edits. */
    window.BZCinematicCard = {
        drawForAlbum: drawPortrait,
        ensureFonts: ensureFonts,
        WIDTH: PW,
        HEIGHT: PH
    };

    // Prewarm: kick off the Playfair Display fetch now, in the background,
    // instead of paying for it the first time someone opens the share shee
    if (document.readyState === 'loading') {
